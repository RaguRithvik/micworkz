...
export default function Setup() {
  ...
  const [showCol, setShowCol] = useState({
    [newConstants.ROW_NUMBER]: { is_hide: true, bool: true, label: 'R.No' },
    [newConstants.TXN_REC_NUMBER]: { is_hide: false, bool: true, label: "TXN Rec Number" },
    [newConstants.TXN_DATE]: { is_hide: false, bool: true, label: "TXN Date" },
    [newConstants.TXN_TYPE]: { is_hide: false, bool: true, label: "TXN Type" },
    [newConstants.MOP_NAME]: { is_hide: false, bool: true, label: "MOP Name" },
    [newConstants.MOP_REMARK_FIELD1_CAPTION]: { is_hide: false, bool: true, label: "Caption 1" },
    [newConstants.MOP_REMARK_FIELD2_CAPTION]: { is_hide: false, bool: true, label: "Caption 2" },
    [newConstants.MOP_REMARKS_FIELD1_VALUE]: { is_hide: false, bool: true, label: "Remark 1" },
    [newConstants.MOP_REMARKS_FIELD2_VALUE]: { is_hide: false, bool: true, label: "Remark 2" },
    [newConstants.TXN_REMARKS]: { is_hide: false, bool: true, label: "TXN Remarks" },
    [newConstants.TXN_STATUS]: { is_hide: false, bool: true, label: "TXN Status" },
  ...
  const loadData = async () => {
    setLoader(true);
    let res = await httpPostRequest(getAllExpense(search_key, (currIndex - 1) * pageSize + 1, pageSize));
  ...
  const editLedgerJournal = async (key) => {
    setAddEdit(false);
    let res = await httpPostRequest(getExpenseByKey(key));
    if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
  ...
  async function deleteLedgerJournal() {
    const res = await httpPostRequest(deleteExpense(deleteId));
...
const EditContainer = ({ classes, loadData, editData, setEditData, addEdit, setAddEdit, languages, setAlertMsg }) => {
  ...
  const getMopDetails = async (mopKey) => {
    ...
    let res = await httpPostRequest(getMopKeyModeOfPayExpense(mopKey));
    if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
  ...
  const getAccountDetails = (accountKey) => {
    let transDetails_ = _.cloneDeep(transDetails);
    let filterData_ = mopData.filter(f => f[newConstants.LEDGER_KEY] == accountKey)?mopData.filter(f => f[newConstants.LEDGER_KEY] == accountKey)[0]:null
    setFilterData(filterData_)    
    setAccountName(filterData_!=null?filterData_["ledger-name"]:"")
    if (editData) {
      let obj_={};
  ...
  const save = async () => {
    ...
    if (!localFields_validation.err && !transDetails_validation.err && !mopFields_validator.err) {
      setLoader(true);
      if (editData == null) {
        let res = await httpPostRequest(createExpense(localFields, transDetails, mopFields));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });
          loadData();
          setLoader(false);
        ...
        let res = await httpPostRequest(updateExpense(editData[newConstants.TXN_MASTER_KEY], localFields, transDetails, mopFields),
        );
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });
          loadData();
...
const LedgerGroupParent = ({ name, value, onChange, error, helperText, label }) => {
  ...
  const ledgerGroupByKey = async () => {
    if (value && defaultOptions.length > 0 && defaultOptions.filter((f) => f.value == value).length == 0) {
      const res = await httpPostRequest(getExpenseLedgerGroupType(value));
  ...
  const loadLedgerGroup = async (inputValue = '', callback = null) => {
    const res = await httpPostRequest(getExpenseLedgerGroupType(inputValue));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
      if (callback) {
        callback(
...
const TransTypeKey = ({ name, value, onChange, error, helperText, label }) => {
  ...
  const loadTransType = async (inputValue = '', callback = null) => {
    const res = await httpPostRequest(getAllExpenseTnxType(inputValue));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
      if (callback) {
        callback(
