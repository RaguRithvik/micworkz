...
import { createAttractionTicketAmenityType, updateAttractionTicketAmenityType, deleteAttractionTicketAmenityType, getAttractionTicketAmenityTypeByKey, getAllAttractionTicketAmenityType, } from '../../helper/RequestPayLoadAttractions';
...
export default function Setup() {
  ...
  const loadData = async () => {
    if (!addEdit) {
      setLoader(true);
      let res = await httpPostRequest(getAllAttractionTicketAmenityType(search_key, (currIndex - 1) * pageSize + 1, pageSize));
  ...
  const editAmenity = async (key) => {
    setAddEdit(false)
    let res = await httpPostRequest(getAttractionTicketAmenityTypeByKey(key));
    if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
  ...
  async function deleteAmenity() {
    const res = await httpPostRequest(deleteAttractionTicketAmenityType(deleteId));
...
const EditContainer = ({ classes, loadData, editData, setEditData, addEdit, setAddEdit, setAlertMsg }) => {
  ...
  const save = async () => {
    ...
    if (!localFields_validation.err) {
      setLoader(true);
      if (editData == null) {
        let res = await httpPostRequest(createAttractionTicketAmenityType(localFields, multi_lang_));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });
          loadData();
          setLoader(false);
        ...
        let res = await httpPostRequest(updateAttractionTicketAmenityType(editData[newConstants.TICKET_AMENITY_TYPE_KEY], localFields, multi_language),
        );
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });
          loadData();
