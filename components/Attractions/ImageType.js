...
import { createAttractionImageType, updateAttractionImageType, deleteAttractionImageType, getAttractionImageTypeById, getAllAttractionImageType, } from '../../helper/RequestPayLoadAttractions';
...
export default function Setup() {
  ...
  const loadData = async () => {
    if (!addEdit) {
      setLoader(true)
      let res = await httpPostRequest(getAllAttractionImageType(search_key, (currIndex - 1) * pageSize + 1, pageSize));
  ...
  const editImageType = async (data) => {
    setAddEdit(false)
    let res = await httpPostRequest(getAttractionImageTypeById(data));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
  ...
  const delImageType = async () => {
    if (deleteId != '') {
      const res = await httpPostRequest(deleteAttractionImageType(deleteId));
...
const EditContainer = ({ classes, loadData, editData, setEditData, addEdit, setAddEdit, setAlertMsg }) => {
  ...
  const save = async () => {
    ...
    if (!localFields_validation.err) {
      setLoader(true);
      if (editData == null) {
        let res = await httpPostRequest(createAttractionImageType(localFields, multi_lang_));
        ...
        let res = await httpPostRequest(updateAttractionImageType(editData[newConstants.TICKET_IMAGE_TYPE_KEY], localFields, multi_lang_));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });
          setLoader(false);
          setAddEdit(false);
