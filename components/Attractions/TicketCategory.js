...
import { createAttractionTicketCategory, getAllAttractionTicketCategory, deleteAttractionTicketCategoryKey, updateAttractionHotelTicketCategory, getAttractionTicketCategoryKey, } from '../../helper/RequestPayLoadAttractions';
...
export default function Setup() {
  ...
  const loadData = async () => {
    if (!addEdit) {
      setLoader(true);
      let res = await httpPostRequest(getAllAttractionTicketCategory(search_key, (currIndex - 1) * pageSize + 1, pageSize));
  ...
  const editHotelCat = async (key) => {
    let res = await httpPostRequest(getAttractionTicketCategoryKey(key));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
  ...
  const delHotelCat = async () => {
    if (deleteId != '') {
      const res = await httpPostRequest(deleteAttractionTicketCategoryKey(deleteId));
...
const EditContainer = ({ classes, editData, setEditData, addEdit, setAddEdit, setAlertMsg }) => {
  ...
  const save = async () => {
    ...
    if (!localFields_validation.err) {
      setLoader(true);
      if (editData == null) {
        let res = await httpPostRequest(createAttractionTicketCategory(localFields, multi_lang_));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setLoader(false);
          setAddEdit(false);
          setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });
        ...
        let res = await httpPostRequest(updateAttractionHotelTicketCategory(editData[newConstants.TICKET_CATEGORY_KEY], localFields, multi_lang_),
        );
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setLoader(false);
          setAddEdit(false);
