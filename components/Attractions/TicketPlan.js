...
export default function Setup() {
  ...
  const loadData = async () => {
    setLoader(true);
    let res = await httpPostRequest(getAttractionTicketPlan(search_key, (currIndex - 1) * pageSize + 1, pageSize));
  ...
  const editRoomPlan = async (key) => {
    setAddEdit(false);
    let res = await httpPostRequest(getAttractionTicketPlanByKey(key));
  ...
  async function delRoomPlan() {
    const res = await httpPostRequest(deleteAttractionTicketPlan(deleteId));
...
const EditContainer = ({ classes, loadData, editData, setEditData, addEdit, setAddEdit, languages, setAlertMsg ,copylanguages}) => {
  ...
  useEffect(() => {
    if (!editData) {
      ...
      setLocalFields({
        ...
        // [newConstants.IS_ALL_DAYS_IN_WEEK]: {
        //   value: editData[newConstants.IS_ALLDAYS_IS_WEEK],
        //   is_require: false,
        //   error: false,
        //   type: 'boolean',
        //   err_msg: '',
      ...
      let planrates_ = [];
      if (editData[newConstants.TICKET_PLAN_RATES] && editData[newConstants.TICKET_PLAN_RATES].length) {
        editData[newConstants.TICKET_PLAN_RATES].forEach((value) => {
          planrates_.push({
            [newConstants.TICKET_TYPE_KEY]: {
            ...
            [newConstants.TICKET_ALLOTMENT]: {
              value:value[newConstants.TICKET_ALLOTMENT],
              is_require: true,
              error: false,
              type: 'number',
              err_msg: '',
            ...
            [newConstants.ADDITIONAL_TYPE]: {
              value: value[newConstants.ADDITIONAL_TYPE],
              is_require: false,
              error: false,
              type: 'boolean',
              err_msg: '',
            ...
            [newConstants.SERVICE_TYPE]: {
              value: value[newConstants.SERVICE_TYPE],
              is_require: false,
              error: false,
              type: 'boolean',
              err_msg: '',
            ...
            [newConstants.TAX_TYPE]: {
              value: value[newConstants.TAX_TYPE],
              is_require: false,
              error: false,
              type: 'boolean',
              err_msg: '',
  ...
  function removeMulti(index_, tag, index1) {
    if (tag == 'lang') {
      ...
      let planrates_ = _.cloneDeep(planrates);
      if (planrates_.length > 1) {
        planrates_ = planrates_.map((val, index) => (index != index_ ? val : null));
        setPlanrates(planrates_.filter((f) => f != null));
      }
    }
    else if (tag == 'cancel_rules') {
      let cancelPolicyRules_ = _.cloneDeep(cancelPolicyRules);
    ...
    if (flag) {
      ...
      if (editData == null) {
        let res = await httpPostRequest(
          createAttractionTicketPlan(localFields, multi_lang_, amenityLinks, plandescription, planrates, cancelPolicyRules),
        );
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
  ...
  const TickeyKeyAttraction = async (ticketname) => {
    let res = await httpPostRequest(getAttractionTicketByKey(ticketname));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
      setSupplierkey(res[newConstants.DATA][newConstants.SUPPLIER_KEY])
  ...
  useEffect(() => {
    if (ticketname) {
      TickeyKeyAttraction(ticketname)
    }
  }, [ticketname])


  const TicketTypeKey = async (supplierkey) => {
    let res = await httpPostRequest(getKeyAttractionTicketKeyBYSupplier(supplierkey));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
            ...
            <Row>
              <Column md={3} padding={[10, 5]}>
                <TextField
                  ...
                  label={<LanguageConfig id="ticketplan.ticketplanname" />}
              ...
              <Column md={3} padding={[10, 5]}>
                <TicketName
                  ...
                  label={<LanguageConfig id="ticket key" />}
                />
              </Column>
              <Column  md={3} padding={[10, 5]}>
                <TextField
                  type="number"
                  label={<LanguageConfig id="max allowed" />}
              ...
              <Column  md={3} padding={[10, 5]}>
                <TextField
                  ...
                  label={<LanguageConfig id="term date" />}
                  name={newConstants.TICKET_TERM_DATE}
                  value={localFields[newConstants.TICKET_TERM_DATE].value}
                  onChange={stateUpdater}
                  error={
              ...
              {/* <Column md={3} padding={[10, 5]}>
                <TextField
                  type="date"
                  id="room-plan-name-id"
                  label={<LanguageConfig id="ticketplan.expiredate" />}
                  InputLabelProps={{
                    shrink: true,
                    classes: {
                      root: classes.DateTime,
                  ...
                  onChange={stateUpdater}
                  error={
                    localFields[newConstants.EXPIRE_DATE].error && localFields[newConstants.EXPIRE_DATE].is_require
                  }
                  helperText={
                    localFields[newConstants.EXPIRE_DATE].error && localFields[newConstants.EXPIRE_DATE].is_require
              ...
              {/* <Row>
                ...
                <Column md={3} padding={[10, 20]} center>
                  <FormControlLabel
                    ...
                    label={<LanguageConfig id="ticketplan.isalldaysinweek" />}
                  />
                </Column>
                <Column md={3} padding={[10, 20]} center>
                  <FormControlLabel
                    ...
                    label={<LanguageConfig id="ticketplan.issunday" />}
                  />
                </Column>
                <Column md={3} padding={[10, 20]} center>
                  <FormControlLabel
                    ...
                    label={<LanguageConfig id="ticketplan.ismonday" />}
                  />
                </Column>
                <Column md={3} padding={[10, 20]} center>
                  <FormControlLabel
                    ...
                    label={<LanguageConfig id="ticketplan.istuesday" />}
                  />
                </Column>
                <Column md={3} padding={[10, 20]} center>
                  <FormControlLabel
                    ...
                    label={<LanguageConfig id="ticketplan.iswednesday" />}
                  />
                </Column>
                <Column md={3} padding={[10, 20]} center>
                  <FormControlLabel
                    ...
                    label={<LanguageConfig id="ticketplan.isthursday" />}
                  />
                </Column>
                <Column md={3} padding={[10, 20]} center>
                  <FormControlLabel
                    ...
                    label={<LanguageConfig id="ticketplan.isfriday" />}
                  />
                </Column>
                <Column md={3} padding={[10, 20]} center>
                  <FormControlLabel
                    ...
                    label={<LanguageConfig id="ticketplan.issaturday" />}
                  />
                </Column>
              </Row> */}
              <Column padding={[5]}>
...
const TicketName = ({ name, value, onChange, error, helperText, label }) => {
  ...
  const hotelByKey = async () => {
    if (value && defaultOptions.length > 0 && defaultOptions.filter((f) => f.value == value).length == 0) {
      const res = await httpPostRequest(getAttractionTicketByKey(value));
      if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
        setSetectedValue({
  ...
  const loadHotel = async (inputValue = '', callback = null) => {
    const res = await httpPostRequest(getAllAttractionTicketY(inputValue));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
      if (callback) {
        callback(
...
const TicketTypeName = ({ name, value, onChange, error, helperText, label, tickeykey }) => {
  const [defaultOptions, setDefaultOptions] = useState([]);
...
const RoomPlanLang = ({ multi_language, languages, classes, multiStateUpdater, addMulti, removeMulti }) => {
  const { Language } = DemandDropDown;
  return (
    <Row margin={[10, 5]}>
      <Column padding={[5, 0]}>
        <Text size={14} bold>
          <LanguageConfig id="ticketplan.ticketplanmultilanguages" />
        </Text>
      </Column>
            ...
            <Row>
              ...
              <Column padding={[5]}>
                <TextField
                  ...
                  id="room-plan-name-id"
                  label={<LanguageConfig id="ticketplan.ticketplanname" />}
                  value={value[newConstants.TICKET_PLAN_NAME].value}
                  name={newConstants.TICKET_PLAN_NAME}
                  error={value[newConstants.TICKET_PLAN_NAME].is_require && value[newConstants.TICKET_PLAN_NAME].error}
                  onChange={(e) => multiStateUpdater(e, index, 'lang')}
                  helperText={
                    value[newConstants.TICKET_PLAN_NAME].is_require && value[newConstants.TICKET_PLAN_NAME].error
                      ? value[newConstants.TICKET_PLAN_NAME].err_msg
                      : ''
                  }
                  variant="outlined"
                />
              ...
              <Column
                ...
                <Row className={classes.endPadd}>
                  ...
                  {multi_language.length - 1 == index
                    ? multi_language.length < languages.length && (
                      <Button onClick={() => addMulti('lang')} size="small" variant="contained" className={classes.add}>
                        <Add />
                      </Button>
                    )
                    : null}
...
const TicketPlanAmenityLinks = ({ amenityLinks, classes, multiStateUpdater, addMulti, removeMulti, amenity }) => {
  ...
  return (
    <Row margin={[10, 5]}>
      <Column padding={[5, 0]}>
        <Text size={14} bold>
          <LanguageConfig id="ticketplan.ticketplanamenitylinks" />
        </Text>
      </Column>
      <Column  padding={[5]}>
      <Card padding={[10]} className={classes.box}>
            ...
            <Row>
              <Column md={3} padding={[5]}>
                <TicketAmenityName
                  ...
                  label={<LanguageConfig id="ticketplan.ticketamenity" />} />
              </Column>
              <Column md={3} padding={[5]}>
                <TextField
                  type="number"
                  inputProps={{ min: 0 }}
                  label={<LanguageConfig id="ticketplan.ticketamenityprice" />}
              ...
              <Column md={3} padding={[5]} center middle> 
                <FormControlLabel
                  control={
                    <Checkbox
                  ...
                  label={<LanguageConfig id="ticketplan.isshow" />}
                />
              </Column>
              <Column
...
const TicketPlanDesc = ({ plandescription, classes, multiStateUpdater, addMulti, removeMulti, languages }) => {
  ...
  return (
    <Row margin={[10, 5]}>
      <Column padding={[5, 0]}>
        <Text size={14} bold>
          <LanguageConfig id="ticketplan.ticketplandescription" />
        </Text>
      </Column>
        ...
        <Column md={4} padding={[5]} key={'amenity_' + index}>
          <Card padding={[10]} className={classes.box}>
            <Row>
              <Column padding={[5]}>
                <Language
                  options={languages.filter(
                    (f) =>
                      f.value == value[newConstants.LANG_CODE].value ||
                      !plandescription.map((val) => val[newConstants.LANG_CODE].value).includes(f.value),
                  )}
                  value={value[newConstants.LANG_CODE].value}
                  name={newConstants.LANG_CODE}
                  error={value[newConstants.LANG_CODE].is_require && value[newConstants.LANG_CODE].error}
              ...
              <Column padding={[5]}>
                <TextField
                  label={<LanguageConfig id="ticketplan.ticketlabel" />}
              ...
              <Column padding={[5]}>
                <TextField
                  label={<LanguageConfig id="ticketplan.tickettext" />}
...
const TicketPlanrates = ({ planrates, classes, multiStateUpdater, addMulti, removeMulti, tickeykey ,amenityCharge}) => {

  return (
    <Row margin={[10, 5]}>
      <Column padding={[5, 0]}>
        <Text size={14} bold>
          <LanguageConfig id="Ticket plan rates" />
        </Text>
      </Column>      
      {planrates.map((value, index) => (
        <Column >   
        <Card padding={[10]} className={classes.box} margin={[6,0,0,0]}>       
            <Row>
              <Column md={3} padding={[5]}>
                <TicketTypeName
                  tickeykey={tickeykey}
                  name={newConstants.TICKET_TYPE_KEY}
                  value={value[newConstants.TICKET_TYPE_KEY].value}
                  onChange={(e) => multiStateUpdater(e, index, 'rates')}
                  error={value[newConstants.TICKET_TYPE_KEY].error && value[newConstants.TICKET_TYPE_KEY].is_require}
                  helperText={
                    value[newConstants.TICKET_TYPE_KEY].error && value[newConstants.TICKET_TYPE_KEY].is_require
                      ? value[newConstants.TICKET_TYPE_KEY].err_msg
                      : ''
                  }
              ...
              <Column md={3} padding={[5]}>
                <TextField
                  type="number"
                  inputProps={{ min: 0 }}
                  label={<LanguageConfig id="Ticket allotment" />}
                  value={value[newConstants.TICKET_ALLOTMENT].value}
                  name={newConstants.TICKET_ALLOTMENT}
                  error={value[newConstants.TICKET_ALLOTMENT].is_require && value[newConstants.TICKET_ALLOTMENT].error}
                  onChange={(e) => multiStateUpdater(e, index, 'rates')}
                  helperText={
                    value[newConstants.TICKET_ALLOTMENT].is_require && value[newConstants.TICKET_ALLOTMENT].error
                      ? value[newConstants.TICKET_ALLOTMENT].err_msg
                      : ''
                  }
                  variant="outlined"
                />
              ...
              <Column md={3} padding={[5]}>
                <TextField
                  ...
                  disabled
                  value={value[newConstants.AMENITY_VALUE].value=amenityCharge ? amenityCharge : ''}
              ...
              <Column md={3} padding={[5]}>
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={value[newConstants.ADDITIONAL_TYPE].value}
                      value={value[newConstants.ADDITIONAL_TYPE].value}
                      color="primary"
                      onChange={(e) => multiStateUpdater(e, index, 'rates_check')}
                      name={newConstants.ADDITIONAL_TYPE}
                    />
                  }
                  label={<LanguageConfig id="addition type" />}
                />
              </Column>
              <Column md={3} padding={[5]}>
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={value[newConstants.SERVICE_TYPE].value}
                      value={value[newConstants.SERVICE_TYPE].value}
                      color="primary"
                      onChange={(e) => multiStateUpdater(e, index, 'rates_check')}
                      name={newConstants.SERVICE_TYPE}
                    />
                  }
                  label={<LanguageConfig id="service type" />}
                />
              </Column>
              <Column md={3} padding={[5]}>
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={value[newConstants.TAX_TYPE].value}
                      value={value[newConstants.TAX_TYPE].value}
                      color="primary"
                      onChange={(e) => multiStateUpdater(e, index, 'rates_check')}
                      name={newConstants.TAX_TYPE}
                    />
                  }
                  label={<LanguageConfig id="Tax type" />}
                />
              </Column>
              <Column
                md={planrates.length > 1 && planrates.length - 1 == index ? 9 : 9}
                xs={planrates.length > 1 && planrates.length - 1 == index ? 12 : 12}
                sm={planrates.length > 1 && planrates.length - 1 == index ? 12 : 12}>
                <Row className={classes.endPadd}>
                  {planrates.length > 1 ? (
                    <Button
                      onClick={() => removeMulti(index, 'rates')}
                      className={classes.addEdit}
                      size="small"
                      variant="contained"
                      color="primary">
                  ...
                  {planrates.length - 1 == index ? planrates.length < tickeykey.length && (
                    <Button onClick={() => addMulti('rates')} size="small" variant="contained" className={classes.add}>
                      <Add />
                    </Button>
                  ) : null}
                </Row>
...
const RateCommentsKey = ({ name, value, onChange, error, helperText, label, isDisabled }) => {
  ...
  const loadRate = async (inputValue = '', callback = null) => {
    const res = await httpPostRequest(GetAttractionTicketComments(inputValue));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
      setDefaultOptions(
...
const TicketPlanRefund = ({ cancelPolicies, index, classes, multiStateUpdater, addMulti, removeMulti }) => {
  return (
    <Row margin={[0, 5]}>
      <Column padding={[5, 0]}>
        <Text size={14} bold>
          <LanguageConfig id="ticketplan.ticketplancancelpolicies" />
        </Text>
      </Column>
      <Column padding={[5]}>
      <Card padding={[10]}>
...
const TicketPlanCancelPolicyRules = ({ cancelPolicyRules, classes, multiStateUpdater, addMulti, removeMulti }) => {
  return (
    <Row margin={[10, 5]}>
      <Column padding={[5, 0]}>
        <Text size={14} bold>
          <LanguageConfig id="ticketplan.ticketplancancelpolicyrules" />
        </Text>
      </Column>
            ...
            <Row>
              <Column md={3} padding={[10, 5]}>
                <TextField
                  ...
                  label={<LanguageConfig id="ticketplan.ticketcancelrulename" />}
...
const LanguageContainer = ({ classes, multi_language, multiStateUpdater, languages, copylanguages }) => {
  ...
  return (
    <div ref={ref}>    
        <Row>
          <Column md={12}>
            <Text size={14} bold>Ticket languages</Text>
           </Column> 
