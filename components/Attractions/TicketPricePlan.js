...
export default function Setup() {
  ...
  async function delRoomPlan() {
    const res = await httpPostRequest(deleteAttractionTicketPlanPrice(deleteId));
...
const EditContainer = ({ classes, loadData, editData, setEditData, addEdit, setAddEdit, languages, setAlertMsg }) => {
  ...
  const save = async () => {
    ...
    if (!flag) {
      setLoader(true);
      if (editData == null) {
        let res = await httpPostRequest(AttractionticketPlanPriceSave(localFields));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });
          loadData();
          setLoader(false);
        ...
        let res = await httpPostRequest(AttractionticketPlanPriceUpdate(editData[newConstants.TICKET_SEASONALITY_KEY], localFields));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });
          loadData();
          setLoader(false);
  ...
  const getAttractionTicketPlanByKey = async (planname) => {
    let res = await httpPostRequest(AttractionTicketPlanByTicketKey(planname));
  ...
  useEffect(() => {
    if (planname) {
      getAttractionTicketPlanByKey(planname)
    }
  }, [planname])
...
const TicketName = ({ name, value, onChange, error, helperText, label }) => {
  ...
  const loadOption = async (inputValue = '', callback = null) => {
    const res = await httpPostRequest(getAllAttractionTicketActiveY(inputValue));
    console.log(res)
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
      if (callback) {
        callback(
