...
import { createAttractionTicketTag, getAllAttractionTicketTag, deleteAttractionTickeTag, getAttractionTicketTagInfoById, editAttractionTicketTag, } from '../../helper/RequestPayLoadAttractions';
...
export default function Setup() {
  ...
  const [showCol, setShowCol] = useState({
    ...
    [newConstants.TICKET_TAG_NAME]: { is_hide: false, bool: true, label: <LanguageConfig id="event.attractiontag" /> },
  ...
  const loadData = async () => {
    if (!addEdit) {
      setLoader(true);
      let res = await httpPostRequest(getAllAttractionTicketTag(search_key, (currIndex - 1) * pageSize + 1, pageSize));
  ...
  const editRoomTag = async (key) => {
    let res = await httpPostRequest(getAttractionTicketTagInfoById(key));
    if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
  ...
  async function delRoomTag() {
    if (deleteId != '') {
      const res = await httpPostRequest(deleteAttractionTickeTag(deleteId));
...
const EditContainer = ({ classes, editData, setEditData, addEdit, setAddEdit, setAlertMsg }) => {
  ...
  const save = async () => {
    ...
    if (!localFields_validation.err) {
      setLoader(true);
      if (editData == null) {
        let res = await httpPostRequest(createAttractionTicketTag(localFields, multi_lang_));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setLoader(false);
          setAddEdit(false);
          setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });
        ...
        let res = await httpPostRequest(editAttractionTicketTag(editData['ticket-tag-key'], localFields, multi_lang_));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setLoader(false);
          setAddEdit(false);
          setEditData(null);
