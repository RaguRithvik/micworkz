...
import { getAllAttractionTicketType, getAttractionTicketTypeByKey, deleteAttractionTicketType, createAttractionTicketType, updateAttractionTicketType, } from '../../helper/RequestPayLoadAttractions';
...
export default function Setup() {
  ...
  const [showCol, setShowCol] = useState({
    ...
    [newConstants.TICKET_TYPE_NAME]: { is_hide: false, bool: true, label: <LanguageConfig id="event.generalname" /> },
    [newConstants.IS_ZERO_COST]: { is_hide: false, bool: true, label: <LanguageConfig id="event.iszerocost" /> },
  ...
  const loadData = async () => {
    setLoader(true);
    let res = await httpPostRequest(getAllAttractionTicketType(search_key, (currIndex - 1) * pageSize + 1, pageSize));
    if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
      setData(res[constants.DATA][newConstants.TICKET_TYPES]);
      setLoader(false);
      setMaxPage(
  ...
  const editTicketType = async (key) => {
    setAddEdit(false);
    window.scrollTo({ top: 0, behavior: 'smooth' });
    let res = await httpPostRequest(getAttractionTicketTypeByKey(key));
    if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
  ...
  async function delTicketType() {
    const res = await httpPostRequest(deleteAttractionTicketType(deleteId));
  ...
  return (
    <div>
      ...
      <PrimaryContainer
        ...
        editRow={editTicketType}
        deleteRow={setDeleteId}
        action_key={newConstants.TICKET_TYPE_KEY}
        loader={loader}
        deleteLoader={deleteLoader}
        setEditData={setEditData}>
        <CustomAlert
          message={<LanguageConfig id="tickettype.deletemsg" />}
          open={deleteId != ''}
          setOpen={setDeleteId}
          action={delTicketType}
        />
...
const EditContainer = ({ loadData, classes, editData, setEditData, addEdit, setAddEdit, setAlertMsg }) => {
  ...
  useEffect(() => {
    if (!editData) {
      ...
      setLocalFields({
        [newConstants.TICKET_TYPE_NAME]: {
          value: editData[newConstants.TICKET_TYPE_NAME],
          is_require: true,
          error: false,
          min_length: 2,
          max_length: null,
        ...
        [newConstants.IS_ZERO_COST]: {
          value: editData[newConstants.IS_ZERO_COST],
          is_require: true,
          error: false,
          type: 'boolean',
          err_msg: '',
      ...
      if (editData[newConstants.TICKET_LANGUAGES] && editData[newConstants.TICKET_LANGUAGES].length) {
        editData[newConstants.TICKET_LANGUAGES].forEach((value) => {
          multi_language_.push({
            [newConstants.TICKET_TYPE_NAME]: {
              value: value[newConstants.TICKET_TYPE_NAME],
              is_require: false,
              error: false,
              type: 'text',
              err_msg: '',
  ...
  const save = async () => {
    ...
    let multi_lang_ = multi_language.filter(f => f[newConstants.TICKET_TYPE_NAME].value != "")
    if (!localFields_validation.err) {
      setLoader(true);
      if (editData == null) {
        let res = await httpPostRequest(createAttractionTicketType(localFields, multi_lang_));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });
          loadData();
          setLoader(false);
        ...
        let res = await httpPostRequest(updateAttractionTicketType(editData[newConstants.TICKET_TYPE_KEY], localFields, multi_lang_),
        );
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });;
          loadData();
  ...
  return (
    <div>
      {localFields.hasOwnProperty(newConstants.TICKET_TYPE_NAME) ? (
        <Row padding={[10]}>
          <Column>
            <Row>
              <Column md={3} padding={[10, 5]}>
                <TextField
                  label={<LanguageConfig id="event.tickettypename" />}
                  name={newConstants.TICKET_TYPE_NAME}
                  value={localFields[newConstants.TICKET_TYPE_NAME].value}
                  onChange={stateUpdater}
                  error={
                    localFields[newConstants.TICKET_TYPE_NAME].error
                  }
                  helperText={
                    localFields[newConstants.TICKET_TYPE_NAME].err_msg
                  }
                  required={localFields[newConstants.TICKET_TYPE_NAME].is_require}
                />
              </Column>
              <Column md={3} padding={[10, 20]} center>
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={localFields[newConstants.IS_ZERO_COST].value}
                      color="primary"
                      onChange={() =>
                        setLocalFields({
                          ...localFields,
                          [newConstants.IS_ZERO_COST]: {
                            ...localFields[newConstants.IS_ZERO_COST],
                            value: !localFields[newConstants.IS_ZERO_COST].value,
                          },
                        })
                      }
                  ...
                  label={<LanguageConfig id="event.iszerocost" />}
                />
              </Column>
              ...
              <Row>
                <Column md={6} padding={[7]}>
                  <GeneralLanguageContainer
                    ...
                    constant={newConstants.TICKET_TYPE_NAME}
                    fieldLabel='event.tickettypename'
                  />
                </Column>
              </Row>
              <Column right>
