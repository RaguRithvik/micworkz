...
export default function Setup() {
  ...
  const [showCol, setShowCol] = useState({
    ...
    [newConstants.TICKET_NAME]: { is_hide: false, bool: true, label: <LanguageConfig id="event.attractionname" /> },
    [newConstants.TICKET_CODE]: { is_hide: false, bool: true, label: <LanguageConfig id="event.attractioncode" /> },
  ...
  const loadData = async () => {
    setLoader(true);
    let res = await httpPostRequest(getAllAttractionTicket(search_key, (currIndex - 1) * pageSize + 1, pageSize));
  ...
  const editTicket = async (key) => {
    setAddEdit(false);
    let res = await httpPostRequest(getAttractionTicketByKey(key));
    if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
  ...
  async function delTicket() {
    const res = await httpPostRequest(deleteAttractionTicket(deleteId));
...
const EditContainer = ({ classes, loadData, editData, setEditData, addEdit, setAddEdit, languages, setAlertMsg, copylanguages }) => {
  ...
  const [loader, setLoader] = useState(false);
  const [rightAmenErr, setrightAmenErr] = useState("");
  ...
  useEffect(() => {
    if (!editData) {
      ...
      setLocalFields({
        ...
        [newConstants.IS_STOCK_TICKET]: {
          value: editData[newConstants.IS_STOCK_TICKET],
          is_require: false,
          error: false,
          type: 'boolean',
          err_msg: '',
      ...
      if (editData[newConstants.TICKET_IMAGE_LINKS].length) {
        setTicketImages(
          editData[newConstants.TICKET_IMAGE_LINKS].map((value) => ({
            ...
            url: value[newConstants.TICKET_IMAGE_PATH],
  ...
  const save = async () => {
    ...
    if (!rightAmen.length) {
      setrightAmenErr("Image not found")
    }
    else if (rightAmen.length) {
      setrightAmenErr("")
    }
    let flag =
      !localFields_validation.err &&
      multi_language_validator.filter((f) => f.err).length == 0 &&
      ticket_images.length > 0 && rightAmen.length > 0;
    let multi_lang_ = multi_language.filter(f => f[newConstants.TICKET_NAME].value != "")

    if (flag) {
      setLoader(true);
      if (editData == null) {
        let res = await httpPostRequest(createAttractionTicket(localFields, multi_lang_, rightAmen, ticket_images, ticket_description));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
        ...
        let res = await httpPostRequest(
          updateAttractionTicket(editData[newConstants.TICKET_KEY], localFields, multi_lang_, rightAmen, ticket_images, ticket_description),
        );
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
    ...
    <div>
      {localFields.hasOwnProperty(newConstants.TICKET_NAME) ? (
        <Row padding={[10]}>
          {/* <Column padding={[8]}>
            <Text bold size={16}>
              <LanguageConfig id={editData ? "ticket.edittickets" : "ticket.addtickets"} />
            </Text>
          </Column> */}
          <Column padding={[10]}>
            <Row>
              <Column md={3} padding={[10, 5]}>
                <TicketSuppliers
                  ...
                  label={<LanguageConfig id={"event.supplier"} />}
                />
              </Column>
              <Column md={3} padding={[10, 5]}>
                <TicketCategory
                  label={<LanguageConfig id={"event.category"} />}
              ...
              <Column md={3} padding={[10, 5]}>
                <TextField
                  label={<LanguageConfig id={"general.name"} />}
              ...
              <Column md={3} padding={[10, 5]}>
                <TextField
                  label={<LanguageConfig id={"event.code"} />}
              ...
              <Column md={3} padding={[10, 5]}>
                <TextField
                  ...
                  helperText={
                    localFields[newConstants.BEGIN_DATE].err_msg
                  }
              ...
              <Column md={3} padding={[10, 5]}>
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={localFields[newConstants.IS_STOCK_TICKET].value}
                      color="primary"
                      onChange={() =>
                        setLocalFields({
                          ...localFields,
                          [newConstants.IS_STOCK_TICKET]: {
                            ...localFields[newConstants.IS_STOCK_TICKET],
                            value: !localFields[newConstants.IS_STOCK_TICKET].value,
                          },
                        })
                      }
                      name={newConstants.IS_STOCK_TICKET}
                    />
                  }
                  label={<LanguageConfig id="Is stock ticket" />}
                />
              </Column>
              <Column md={3} padding={[10, 5]}>
                <FormControlLabel
...
const TicketCategory = ({ name, value, onChange, error, helperText, label }) => {
  ...
  const loadTicketCategory = async (inputValue = '', callback = null) => {
    const res = await httpPostRequest(getAllAttractionTicketCategoryY(inputValue));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
      if (callback) {
        callback(
  ...
  return (
    <div ref={ref} style={{ textAlign: "center" }}>
      <Button onClick={() => setOpen(true)} className={classes.blockButton} variant="contained" color="primary">
        <LanguageConfig id="general.languages" />
      </Button>
...
const HotelAmenity = ({ leftAmen, setLeftamen, rightAmen, setRightamen }) => {
    ...
    <Paper className={classes.paper}>
      {tag == "left" ?
        <TextField
          ...
          value={search}
          name="search"
          label={<LanguageConfig id="general.search" />}
  ...
  return (
    <div style={{ textAlign: "center" }}>
      <Button onClick={() => setOpen(true)} className={classes.blockButton} variant="contained" color="primary">
        <LanguageConfig id="event.attractionamenities" />
      </Button>
        ...
        >
          <Row>
            <Column middle center>
              <Text size={23} bold><LanguageConfig id="event.attractionamenities" /></Text>
            </Column>
          </Row>
...
const TicketTag = ({ values, classes, stateUpdater, name }) => {
  const [defaultValue, setDefaultValue] = useState([]);
  useEffect(() => {
    loadOptions();
  }, []);
  const loadOptions = async (inputValue = '', callback = null) => {
    let res = await httpPostRequest(getAllAttractionTicketTagY(inputValue));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
      if (callback) {
        callback(
    ...
    <Row>
      ...
      <Column padding={[5, 0]}>
        <SingelSelectOnDemand
          ...
          placeholder={<LanguageConfig id="Ticket tags" />}
...
const ImageHandler = ({ classes, multiStateUpdater, stateUpdater, ticket_images, localFields, removeMulti }) => {
  ...
  const loadImageType = async () => {
    let res = await httpPostRequest(getAllAttractionImageType(''));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
      setDefaultOptions(
  ...
  const onUploadImage = async (e) => {
    ...
    let images_ = [];
    while (i < e.length) {
      const res = await httpPostRequestWithForm(imageUploder(e[i], 'ATTRACTION_IMAGES', key));
      if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
  ...
  return (
    <Row center>
      <Button onClick={() => setOpen(true)} className={classes.blockButton} variant="contained" color="primary">
        <LanguageConfig id="Ticket Gallery" />
      </Button>
      <ModalComponent open={open} setOpen={setOpen} >
        {/* <LoadingOverlay
        active={loader}
...
const TicketDescription = ({ stateUpdater, multiStateUpdater, removeMulti, classes, ticket_description }) => {
  const options = {
    [newConstants.LANG_CODE]: {
      value: '',
      is_require: true,
  ...
  useEffect(() => {
    ...
    if (editIndex != null) {
      ...
      options_[newConstants.TICKET_DESCRIPTION_TEXT].value = ticket_description[editIndex][newConstants.TICKET_DESCRIPTION_TEXT];
      options_[newConstants.TICKET_DESCRIPTION_LABEL].value = ticket_description[editIndex][newConstants.TICKET_DESCRIPTION_LABEL];
    }
    setValues(options_);
  }, [editIndex]);
                    ...
                    <Column md={3} padding={[5]}>
                      <Language
                        options={languages.filter(
                          (f) =>
                            f.value == values[newConstants.LANG_CODE].value ||
                            !ticket_description.map((val) => val[newConstants.LANG_CODE]).includes(f.value),
                        )}
