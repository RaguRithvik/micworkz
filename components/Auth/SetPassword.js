...
import { verifyEmail, updatePassword } from "../../helper/RequestPayLoad";
...
const SetPassword = ({ variant = 'default', wrapperVariant = 'default' }) => {
    ...
    const getEmailId = async () => {
        let id = router.asPath.replace("/verifyemail/?id=", "");
        if (id) {
    ...
    const onSubmit = async (e) => {
        ...
        if (isOK) {
            setLoader(true);
            const res = await httpPostRequest(updatePassword(email.value.trim(), password.value.trim(), cpassword.value.trim()))
