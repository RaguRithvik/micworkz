...
const HotelListSidebar = ({ loader }) => {
  ...
  return filtersList && filtersList.ammenities ? (
    <div>
      <Row>
        <Column>
          <Card padding={[10]}>
              ...
              <Row>
                ...
                {priceRange.map((value, index) => (
                  <Column key={'price_rnage_' + index} padding={[0, 10]}>
                    <FormControlLabel
                      control={<Checkbox checked={priceIndex.includes(index)} onChange={(e) => selectPrices(index)} />}
                      label={
                        <p className={classes.optionLabel}>
                          {currency} {value.length > 1 ? value[0] + ' - ' + value[1] : value[0]}
                        </p>
                      }
                    />
                  </Column>
                  ...
                  ? filtersList.starRatings.map((value, index) => (
                      <Column padding={[0, 10]}>
                        <FormControlLabel
                          control={
                            <Checkbox
                              checked={starRating.includes(value.hotelCategoryGroupId)}
                              onChange={(e) => selectRating(value.hotelCategoryGroupId)}
                            />
                          }
                          label={<p className={classes.optionLabel}>{value.hotelCategoryGroupName}</p>}
                        />
                      </Column>
                    ))
                  : null}
                ...
                {filtersList.ammenities.map((value, index) => (
                  <Column key={'ammenities_' + index} padding={[0, 10]}>
                    <FormControlLabel
                      className={{ label: classes.ckeckBoxLabel }}
                      control={
                        <Checkbox
                          checked={ammunitiesm.includes(index)}
                          onChange={(e) => addToAmmunities(value, index)}
                          name={value.amenityName}
                        />
                      }
                      label={<p className={classes.optionLabel}>{value.amenityName}</p>}
                    />
                  </Column>
                ))}
              </Row>
