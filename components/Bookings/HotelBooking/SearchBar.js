...
import { autoComplete } from '../../../helper/RequestPayLoadBookings';
...
export default function SearchBar() {
  ...
  useEffect(() => {
    if (router.query && router.query.ci && router.query.ci) {
      setValue(
        moment.range(
          router.query.ci ? ddMMYYYYtoStamp(router.query.ci) : new Date(),
          router.query.co ? ddMMYYYYtoStamp(router.query.co) : new Date(),
        ),
      );
    }
    setOccupString(
      router.query &&
        router.query.rd &&
        router.query.rd.split('-').length == 3 &&
        router.query.rd.split('-')[2] &&
        (router.query.rd.split('-')[2].length == 1 || router.query.rd.split('-')[2].split('_').length > 1)
        ? router.query.rd
        : '1-2-0',
    );
    ...
    setSelected({
      hid: router.query.hid ? router.query.hid : '',
      cg: router.query.cg ? router.query.cg : '',
      hotelKey: router.query.hotelKey ? decodeURI(router.query.hotelKey) : '',
      clientId: router.query.clientId ? router.query.clientId : '',
      productProviderTypeId: router.query.productProviderTypeId ? router.query.productProviderTypeId : '',
      hcod: router.query.hcod ? router.query.hcod : '',
      ...
      router.push({
        pathname:
          selected && selected.cg && selected.cg == 'City'
            ? '/dashboard/Bookings/HotelBooking/'
            : '/dashboard/Bookings/HotelBooking/Room',
        query: {
          name: encodeURI(key),
          ci: CheckinDate,
          hid: selected.hid,
  ...
  return (
    <AppBar position="static" ref={ref} elevation="0" color="inherit">
      <Row variant="dense" className={classes.appBar} center middle>
                ...
                <Touchable onClick={() => setOpen(true)} className={classes.startDateEndDateContainer}>
                  <Row padding={[2, 0]}>
                    ...
                    <Column xs={10} sm={10} md={10}>
                      <Row>
                        <Column padding={[0, 5]}>
                          <Text color="#003399" size={12} bold>
                            {<LanguageConfig id="hotels.startandenddate" />}
                          </Text>
                        </Column>
                        <Column padding={[0, 5]}>
                          <Row>
                            <Text size={12} semibold color="black">
                              {getDateddMMMyyyy(value?.start?._d)}
                            </Text>
                            <Text size={14} color={'#003399'} bold style={{ paddingLeft: 5, paddingRight: 5 }}>
                              <LanguageConfig id="hotels.to" />
                            </Text>
                            <Text size={12} semibold color="black">
                              {getDateddMMMyyyy(value?.end?._d)}
                            </Text>
                          </Row>
                        </Column>
                      </Row>
            ...
            <Column md={4} xl={4} xs={8} sm={8} padding={[5]}>
              <Card className={classes.occupancyContainer} center>
                    ...
                    <Column md={10} xs={10} sm={10}>
                      <Row>
                        <Column>
                          <Text size={14} medium>
                            {' '}
                            {occup_string.split('-').length == 3 ? occup_string.split('-')[1] : ''}{' '}
                            <LanguageConfig id="hotels.adults" />{' '}
                            {occup_string.split('-').length == 3
                              ? occup_string.split('-')[2].split('_').length > 0
                                ? occup_string.split('-')[2].split('_')[0]
                                : ''
                              : ''}{' '}
                            <LanguageConfig id="hotels.children" />{' '}
                          </Text>
                        </Column>
                        <Column>
                          <Text className={classes.rooms} medium>
                            {' '}
                            {occup_string.split('-').length == 3 ? occup_string.split('-')[0] : ''}{' '}
                            <LanguageConfig id="hotels.room" />{' '}
                          </Text>
                        </Column>
                      </Row>
                    </Column>
                ...
                <Occupancy
                  ...
                  occup={occup}
                  classes={classes}
                  occup_string={occup_string}
                  setOccupString={setOccupString}
                />
              </Card>
            </Column>
            <Column md={3} xl={3} xs={4} sm={4} padding={[4, 5]}>
...
const Occupancy = ({ occup, setOccup, occup_string, setOccupString, classes, ...props }) => {
  ...
  useEffect(() => {
    ...
    setChildrenAges(
      occup_array.length == 3
        ? occup_array[2].split('_').length > 1
          ? occup_array[2]
              .split('_')
              .map((val, index) => index > 0 && { value: val, error: false })
              .filter((f) => f)
          : []
        : [],
    );
    ...
    <CustomTooltip open={occup} onClose={() => setOccup(false)} className={classes.occupancyToolpic} {...props}>
      <Row padding={[15]}>
        <Column>
          <IncDecContainer
            defaultValue={1}
            label={<LanguageConfig id="hotels.room" />}
            value={room}
            setState={(val) => setRoom(val)}
          />
        </Column>
        ...
        <Column>
          <IncDecContainer
            defaultValue={2}
            label={<LanguageConfig id="hotels.Adult" />}
            value={adult}
            setState={(val) => setAdult(val)}
          />
        </Column>
        ...
        <Column>
          <IncDecContainer
            defaultValue={0}
            label={<LanguageConfig id="hotels.children" />}
            value={children}
            setState={(val) => updateChildrens(val)}
          />
        </Column>
      ...
      {children_ages && children_ages.length > 0 ? (
        <Row>
          <Column padding={[10]}>
            <Text bold>Age of children</Text>
          </Column>
          <Column>
            <Row padding={[0, 5]}>
      ...
      <Row padding={[0, 0, 10, 0]}>
        ...
        <Column md={8} padding={[20]}>
          <Button variant="contained" color="primary" onClick={submit}>
            <LanguageConfig id="hotels.submit" />
          </Button>
        </Column>
        <Column md={2}></Column>
      </Row>
    ...
    <Row {...prop}>
      ...
      <Column md={5} xs={8} sm={8}>
        <Paper variant="outlined" style={{ borderRadius: 8, margin: 5 }}>
          <Row>
            <Column md={4} xs={4} sm={4}>
              <IconButton onClick={dec}>
                <Remove />
              </IconButton>
            </Column>
            <Column md={4} xs={4} sm={4} center middle>
              <Text center color="#003399" size={18} bold>
                {value}
              </Text>
            </Column>
            <Column md={4} xs={4} sm={4}>
