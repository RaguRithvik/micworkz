...
import { getAllEventTicketAmenityType, getAllEventTicketAmenity, getEventTicketAmenityByKey, deleteEventTicketAmenity, updateEventTicketAmenity, createEventTicketAmenity, getEventTicketAmenityTypeByKey, } from '../../helper/RequestPayLoadEvents';
...
export default function Setup() {
  ...
  const getData = async () => {
    if (!addEdit) {
      setLoader(true);
      let res = await httpPostRequest(getAllEventTicketAmenity(search_key, (currIndex - 1) * pageSize + 1, pageSize));
  ...
  async function edit(key) {
    ...
    let res = await httpPostRequest(getEventTicketAmenityByKey(key));
    if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
  ...
  const deleteAmmunityCall = async () => {
    const res = await httpPostRequest(deleteEventTicketAmenity(deleteId));
...
const EditContainer = ({ classes, setEditData, setAddEdit, editData, setAlertMsg, getData }) => {
  ...
  const save = async () => {
    ...
    if (!localFields_validation.err) {
      setLoader(true);
      if (editData == null) {
        let res = await httpPostRequest(createEventTicketAmenity(localFields, multi_lang_));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });
          getData();
          setLoader(false);
        ...
        let res = await httpPostRequest(updateEventTicketAmenity(editData['ticket-amenity-key'], localFields, multi_lang_));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });
          getData();
          setLoader(false);
...
const TicketAmenityType = ({ name, value, onChange, error, helperText, label }) => {
  ...
  const amenityByKey = async () => {
    if (value && defaultOptions.length > 0 && defaultOptions.filter((f) => f.value == value).length == 0) {
      const res = await httpPostRequest(getEventTicketAmenityTypeByKey(value));
  ...
  const loadAmenity = async (inputValue = '', callback = null) => {
    const res = await httpPostRequest(getAllEventTicketAmenityType(inputValue));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
      if (callback) {
        callback(
