...
import { getAllEventTicketPriceComments, createEventTicketPriceComments, getEventTicketPriceCommentsByKey, updateEventTicketPriceComments, deleteEventTicketPriceComments, } from '../../helper/RequestPayLoadEvents';
...
export default function Setup() {
  ...
  const getData = async () => {
    if (!addEdit) {
      setLoader(true);
      let res = await httpPostRequest(getAllEventTicketPriceComments(search_key, (currIndex - 1) * pageSize + 1, pageSize));
  ...
  async function edit(id) {
    setAddEdit(false);
    let res = await httpPostRequest(getEventTicketPriceCommentsByKey(id));
    if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
  ...
  const deleteRoomCall = async () => {
    const res = await httpPostRequest(deleteEventTicketPriceComments(deleteId));
...
const EditContainer = ({ getData, classes, editData, setEditData, addEdit, setAddEdit, setAlertMsg }) => {
  ...
  const save = async () => {
    ...
    if (!localFields_validation.err) {
      setLoader(true);
      if (editData == null) {
        let res = await httpPostRequest(createEventTicketPriceComments(localFields, multi_lang_));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });
          getData();
          setLoader(false);
        ...
        let res = await httpPostRequest(updateEventTicketPriceComments(editData[newConstants.TICKET_PRICE_COMMENTS_KEY], localFields, multi_lang_),
        );
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });
          getData();
