...
export default function Setup() {
    ...
    const [showCol, setShowCol] = useState({
        ...
        [newConstants.SUPPLIER_NAME]: { is_hide: false, bool: true, label: "Supplier name" },
        ...
        [newConstants.INVOICE_STATUS]: { is_hide: false, bool: true, label: "Invoice status" },
    ...
    const loadData = async () => {
        setLoader(true);
        let res = await httpPostRequest(getAllEventPurchase(search_key, (currIndex - 1) * pageSize + 1, pageSize));
...
const EditContainer = ({ classes, setEditData, languages, addEdit, setAddEdit, editData, setAlertMsg, loadData }) => {
    ...
    const getModuleDetails = async (supplierKey) => {
        let res = await httpPostRequest(EventTicketpurchaseinvoiceKey(supplierKey));
        if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA] != null && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
            ...
            if(res[newConstants.DATA][newConstants.SUPPLIER_TICKETS].length>0 && res[newConstants.DATA][newConstants.SUPPLIER_TICKET_TYPES].length>0){
            ...
            if (res[newConstants.DATA][newConstants.SUPPLIER_TICKET_TYPES] != null && res[newConstants.DATA][newConstants.SUPPLIER_TICKET_TYPES].length>0) {
                ...
                setPurchaseDetails([])
                setAlertMsg({ type: 'error', msg: 'Supplier tickets not found' });
            }
            setTicketPlan([])
            setTicketvalue("")
        }
    ...
    const save = async () => {
        ...
        if (!localFields_validation.err && purchaseDetails_validator.filter((f) => f.err).length == 0) {
            setLoader(true);
            if (editData == null) {
                let res = await httpPostRequest(createEventTicketPurchaseInvoice(localFields, purchaseDetails, purchaseModule == "MODULE_A" ? invoiceExcel : []));
                console.log(res)
                if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
...
const StockName = ({ name, value, onChange, error, helperText, label, stockKeys }) => {
    ...
    useEffect(() => {
        setSelectValue(defaultOptions.filter((f) => f.value == value)[0]);
    }, [value, defaultOptions, stockKeys])
