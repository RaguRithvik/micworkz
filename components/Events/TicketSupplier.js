...
import { createEventTicketSupplier, updateEventTicketSupplier, getEventTicketSupplier, deleteEventTicketSupplier, getTicketModule, getKeyEventTicketSupplier, LedgerAccountSupplier, eventSupplierTaxRule, getAllEventTicketType } from '../../helper/RequestPayLoadEvents';
...
export default function Setup() {
  ...
  const [isDescShow, setIsDescShow] = useState({ active: false, desc: '' })
  ...
  const [showCol, setShowCol] = useState({
    ...
    ["ticket-supplier-country-name"]: { is_hide: false, bool: true, label: <LanguageConfig id="event.eventcode" /> },
  ...
  const loadData = async () => {
    setLoader(true)
    let res = await httpPostRequest(getEventTicketSupplier(search_key, (currIndex - 1) * pageSize + 1, pageSize));
  ...
  const editAmmunity = async (id) => {
    setAddEdit(false);
    let res = await httpPostRequest(getKeyEventTicketSupplier(id));
  ...
  async function deleteAmmunity() {
    const res = await httpPostRequest(deleteEventTicketSupplier(deleteId));
    ...
    <div>
      {isDescShow.active &&
        <Alert severity="info" style={{ marginBottom: '10px' }}>
          <AlertTitle><LanguageConfig id="general.description" /></AlertTitle>
          {isDescShow.desc}
        </Alert>
      }
...
const EditContainer = ({ classes, loadData, editData, languages, setEditData, addEdit, setAddEdit, setAlertMsg, setIsDescShow }) => {
  ...
  useEffect(() => {
    if (!editData) {
      ...
      if (editData[newConstants.TICKET_SUPPLIER_LINKS].length) {
        let array = [];
        editData[newConstants.TICKET_SUPPLIER_LINKS].map((value) => {
          array.push(value[newConstants.TICKET_TYPE_KEY]);
        });
        setDefaultTypes(array);
      }
      setLocalFields({
        [newConstants.TICKET_MODULE_KEY]: {
  ...
  const save = async () => {
    ...
    if (!localFields_validation.err) {
      setLoader(true);
      if (editData == null) {
        let res = await httpPostRequest(createEventTicketSupplier(localFields, multi_lang_));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
        ...
        let res = await httpPostRequest(updateEventTicketSupplier(editData[newConstants.TICKET_SUPPLIER_KEY], localFields, multi_lang_));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
...
const LedgerGroupName = ({ name, value, onChange, error, helperText, label }) => {
  ...
  const loadLedger = async (inputValue = "", callback = null) => {
...
const ModuleName = ({ name, value, onChange, error, helperText, label, setIsDescShow }) => {
  const [defaultOptions, setDefaultOptions] = useState([]);
  const [selectValue, setSetectedValue] = useState(null)
  ...
  const loadModale = async (inputValue = "", callback = null) => {
    ...
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
      if (callback) {
        callback(res[newConstants.DATA][newConstants.TICKET_MODULES].map((v) =>
        ({
          value: v[newConstants.TICKET_MODULE_KEY],
          label: v[newConstants.TICKET_MODULE_DESC],
          desc: v[newConstants.TICKET_MODULE_DESC]
        })));
      }
      else {
        ...
        ({
          ...
          desc: value[newConstants.TICKET_MODULE_DESC]
        })));
      }
    }
  }
  ...
  const onChangeHandler = e => {
    ...
    setIsDescShow({ active: true, desc: e.desc })
    setTimeout(() => { setIsDescShow({ active: false, desc: '' }) }, 3000);
  }
...
const TicketType = ({ values, stateUpdater, defaultValue, setDefaultValue, defaultTypes }) => {

  useEffect(() => {
    loadOptions();
  }, []);
  const loadOptions = async (inputValue = '', callback = null) => {
    let res = await httpPostRequest(getAllEventTicketType(inputValue));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
      if (callback) {
        callback(
    ...
    <Row>
      ...
      <Column padding={[5, 0]}>
          ...
          {defaultValue.map(el => (
            <Column md={2}>
              <FormControlLabel
                control={
                  <Checkbox
                    defaultChecked={defaultTypes.includes(el.value)}
                    color="primary"
                    name={newConstants.TICKET_TYPE_KEY}
                    value={el.value}
                    onChange={(e) => stateUpdater(e, 'checkers', newConstants.TICKET_TYPE_KEY)}
                  />
                }
                label={el.label}
              />
        ...
        {/* <SingelSelectOnDemand
          ...
          placeholder={<LanguageConfig id="Ticket type" />}
