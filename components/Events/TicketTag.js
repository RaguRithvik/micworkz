...
import { createEventTicketTag, getAllEventTicketTag, deleteEventTickeTag, getEventTicketTagInfoById, editEventTicketTag, } from '../../helper/RequestPayLoadEvents';
...
export default function Setup() {
  ...
  const [showCol, setShowCol] = useState({
    ...
    [newConstants.TICKET_TAG_NAME]: { is_hide: false, bool: true, label: <LanguageConfig id="event.eventtag" /> },
  ...
  const loadData = async () => {
    if (!addEdit) {
      setLoader(true);
      let res = await httpPostRequest(getAllEventTicketTag(search_key, (currIndex - 1) * pageSize + 1, pageSize));
      if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
        setData(res[constants.DATA][newConstants.TICKET_TAGS]);
        setLoader(false);
        setMaxPage(
  ...
  const editRoomTag = async (key) => {
    let res = await httpPostRequest(getEventTicketTagInfoById(key));
    if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
  ...
  async function delRoomTag() {
    if (deleteId != '') {
      const res = await httpPostRequest(deleteEventTickeTag(deleteId));
  ...
  return (
    <div>
      ...
      <PrimaryContainer
        ...
        action_key={newConstants.TICKET_TAG_KEY}
        loader={loader}
...
const EditContainer = ({ classes, editData, setEditData, addEdit, setAddEdit, setAlertMsg }) => {
  ...
  useEffect(() => {
    if (!editData) {
      ...
      setLocalFields({
        [newConstants.TICKET_TAG_NAME]: {
          value: editData[newConstants.TICKET_TAG_NAME],
          is_require: true,
          error: false,
          min_length: 2,
          max_length: null,
      ...
      let multi_language_ = [];
      if (editData[newConstants.TICKET_TAG_LANGUAGES] && editData[newConstants.TICKET_TAG_LANGUAGES].length) {
        editData[newConstants.TICKET_TAG_LANGUAGES].forEach((value) => {
          multi_language_.push({
            ...
            [newConstants.TICKET_TAG_NAME]: {
              value: value[newConstants.TICKET_TAG_NAME],
              is_require: false,
              error: false,
              min_length: 1,
              max_length: null,
  ...
  const save = async () => {
    ...
    let multi_lang_ = multi_language.filter(f => f[newConstants.TICKET_TAG_NAME].value != "")
    if (!localFields_validation.err) {
      setLoader(true);
      if (editData == null) {
        let res = await httpPostRequest(createEventTicketTag(localFields, multi_lang_));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setLoader(false);
          setAddEdit(false);
          setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });
        ...
        let res = await httpPostRequest(editEventTicketTag(editData['ticket-tag-key'], localFields, multi_lang_));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setLoader(false);
          setAddEdit(false);
          setEditData(null);
  ...
  return (
    <div>
      {localFields.hasOwnProperty(newConstants.TICKET_TAG_NAME) ? (
        <Row padding={[10]}>
          <Column>
            <Row>
              <Column md={3} padding={[10, 5]}>
                <TextField
                  ...
                  name={newConstants.TICKET_TAG_NAME}
                  value={localFields[newConstants.TICKET_TAG_NAME].value}
                  onChange={stateUpdater}
                  required={localFields[newConstants.TICKET_TAG_NAME].is_require}
                  error={
                    localFields[newConstants.TICKET_TAG_NAME].error &&
                    localFields[newConstants.TICKET_TAG_NAME].is_require
                  }
                  helperText={
                    localFields[newConstants.TICKET_TAG_NAME].error &&
                      localFields[newConstants.TICKET_TAG_NAME].is_require
                      ? localFields[newConstants.TICKET_TAG_NAME].err_msg
                      : ''
                  }
                  variant="outlined"
                />
              ...
              <Row>
                <Column md={6} padding={[7]}>
                  <GeneralLanguageContainer
                    ...
                    constant={newConstants.TICKET_TAG_NAME}
