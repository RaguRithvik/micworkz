...
import { getAllEventTicketType, getEventTicketTypeByKey, deleteEventTicketType, createEventTicketType, updateEventTicketType, } from '../../helper/RequestPayLoadEvents';
...
export default function Setup() {
  ...
  const [showCol, setShowCol] = useState({
    ...
    [newConstants.TICKET_TYPE_NAME]: { is_hide: false, bool: true, label: <LanguageConfig id="event.tickettypename" /> },
  ...
  const loadData = async () => {
    setLoader(true);
    let res = await httpPostRequest(getAllEventTicketType(search_key, (currIndex - 1) * pageSize + 1, pageSize));
  ...
  const editTicketType = async (key) => {
    ...
    let res = await httpPostRequest(getEventTicketTypeByKey(key));
    if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
  ...
  async function delTicketType() {
    const res = await httpPostRequest(deleteEventTicketType(deleteId));
...
const EditContainer = ({ loadData, classes, editData, setEditData, addEdit, setAddEdit, setAlertMsg }) => {
  ...
  const save = async () => {
    ...
    if (!localFields_validation.err) {
      setLoader(true);
      if (editData == null) {
        let res = await httpPostRequest(createEventTicketType(localFields, multi_lang_));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });
          loadData();
          setLoader(false);
        ...
        let res = await httpPostRequest(updateEventTicketType(editData[newConstants.TICKET_TYPE_KEY], localFields, multi_lang_),
        );
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });;
          loadData();
            ...
            <Row>
              <Column md={3} padding={[10, 5]}>
                <TextField
                  ...
                  error={localFields[newConstants.TICKET_TYPE_NAME].error}
                  helperText={localFields[newConstants.TICKET_TYPE_NAME].err_msg}
