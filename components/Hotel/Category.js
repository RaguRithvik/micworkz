...
import { TextField, Text, Card, Row, Column, Loader, Glyphi, CustomAlert, SingelSelectOnDemand, DemandDropDown } from '../../core';
import {
  Fade,
  Button,
  FormControlLabel,
...
export default function Setup() {
  ...
  const { setAlertMsg ,languages, formname} = useStore();
  ...
  const [showCol, setShowCol] = useState({
    [newConstants.ROW_NUMBER]: { is_hide: true, bool: true, label: <LanguageConfig id="hotelcategory.sno" /> },
    [newConstants.HOTEL_CATEGORY_NAME]: { is_hide: false, bool: true, label: <LanguageConfig id="hotelcategory.hotelcategory" /> },
    [newConstants.HOTEL_TYPE_DESC]: { is_hide: false, bool: true, label: <LanguageConfig id="hotelcategory.hoteltype" /> },
    [newConstants.HOTEL_CATEGORY_GLYP_CSS]: { is_hide: false, bool: true, label: <LanguageConfig id="hotelcategory.icon" /> },
    [newConstants.LAST_UPDATED_ON]: { is_hide: false, bool: true, label: <LanguageConfig id="hotelcategory.lastupdatedon" /> },
    [newConstants.LAST_UPDATED_BY]: { is_hide: false, bool: true, label: <LanguageConfig id="hotelcategory.lastupdatedby" /> },
    action: { is_hide: true, show: false, bool: true, label: <LanguageConfig id="hotelcategory.action" /> }
  });

  const loadData = async () => {
    if (!addEdit) {
      setLoader(true);
      let res = await httpPostRequest(getAllHotelCategory(search_key, (currIndex - 1) * pageSize + 1, pageSize));
      if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
        setData(res[newConstants.DATA][newConstants.HOTEL_CATEGORIES]);
        setLoader(false);
        setMaxPage(
  ...
  const editHotelCat = async (key) => {
    setAddEdit(false);
    let res = await httpPostRequest(getHotelCategoryInfoById(key));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
  ...
  const delHotelCat = async () => {
    if (deleteId != '') {
      const res = await httpPostRequest(deleteHotelCategory(deleteId));
  ...
  return (
    <div>
      ...
      <PrimaryContainer
        ...
        action_key={newConstants.HOTEL_CATEGORY_KEY}
        loader={loader}
        deleteLoader={deleteLoader}
        setEditData={setEditData}>
        <CustomAlert
          message={<LanguageConfig id="hotelcategory.deletemsg" />}
          open={deleteId != ''}
          setOpen={setDeleteId}
          action={delHotelCat}
        />
...
const EditContainer = ({ classes, editData, setEditData, addEdit, setAddEdit, setAlertMsg }) => {
  ...
  useEffect(() => {
    if (!editData) {
      ...
      setLocalFields({
        ...
        [newConstants.HOTEL_CATEGORY_NAME]: {
          value: editData[newConstants.HOTEL_CATEGORY_NAME],
          is_require: true,
          error: false,
          min_length: 2,
          max_length: null,
        ...
        [newConstants.HOTEL_CATEGORY_GLYP_CSS]: {
          value: editData[newConstants.HOTEL_CATEGORY_GLYP_CSS],
          is_require: true,
          error: false,
          type: 'dropdown',
          err_msg: '',
      ...
      let multi_language_ = [];
      if (editData[newConstants.HOTEL_CATEGORY_LANGUAGES] && editData[newConstants.HOTEL_CATEGORY_LANGUAGES].length) {
        editData[newConstants.HOTEL_CATEGORY_LANGUAGES].forEach((value) => {
          multi_language_.push({
            ...
            [newConstants.HOTEL_CATEGORY_NAME]: {
              value: value[newConstants.HOTEL_CATEGORY_NAME],
              is_require: true,
              error: false,
              min_length: 1,
              max_length: null,
  ...
  const save = async () => {
    ...
    let multi_lang_ = multi_language.filter(f => f[newConstants.HOTEL_CATEGORY_NAME].value != "")
    if (!localFields_validation.err) {
      setLoader(true);
      if (editData == null) {
        let res = await httpPostRequest(createHotelCategory(localFields, multi_lang_));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setLoader(false);
          setAddEdit(false);
          setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });
        ...
        let res = await httpPostRequest(
          editHotelCategory(editData[newConstants.HOTEL_CATEGORY_KEY], localFields, multi_lang_),
        );
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setLoader(false);
          setAddEdit(false);
      ...
      setAlertMsg({ type: 'error', msg: <LanguageConfig id="hotelcategory.fillrequired" /> });
    }
  };
  ...
  return (
    <div>
      {localFields.hasOwnProperty(newConstants.HOTEL_CATEGORY_NAME) ? (
        <Row padding={[10]}>
          <Column>
            <Row>
              <Column md={3} padding={[10, 5]}>
                <TextField
                  ...
                  value={localFields[newConstants.HOTEL_CATEGORY_NAME].value}
                  onChange={stateUpdater}
                  error={localFields[newConstants.HOTEL_CATEGORY_NAME].error}
                  helperText={localFields[newConstants.HOTEL_CATEGORY_NAME].err_msg}
                  required={localFields[newConstants.HOTEL_CATEGORY_NAME].is_require}
                />
              </Column>
              <Column md={3} padding={[10, 5]}>
                <HotelDetail
                  ...
                  error={
                    localFields[newConstants.HOTEL_TYPE_KEY].is_require && localFields[newConstants.HOTEL_TYPE_KEY].error
                  }
                  onChange={stateUpdater}
                  helperText={
                    localFields[newConstants.HOTEL_TYPE_KEY].is_require && localFields[newConstants.HOTEL_TYPE_KEY].error
                      ? localFields[newConstants.HOTEL_TYPE_KEY].err_msg
                      : ''
                  }
                  label={<LanguageConfig id={"hotelcategory.type"} />}
                />
              </Column>
              <Column md={3} padding={[10, 5]}>
                <Glyphi
                  ...
                  value={localFields[newConstants.HOTEL_CATEGORY_GLYP_CSS].value}
                  error={
                    localFields[newConstants.HOTEL_CATEGORY_GLYP_CSS].error &&
                    localFields[newConstants.HOTEL_CATEGORY_GLYP_CSS].is_require
                  }
                  onChange={stateUpdater}
                  helperText={
                    localFields[newConstants.HOTEL_CATEGORY_GLYP_CSS].error &&
                      localFields[newConstants.HOTEL_CATEGORY_GLYP_CSS].is_require
                      ? localFields[newConstants.HOTEL_CATEGORY_GLYP_CSS].err_msg
                      : ''
                  }
                  label={<LanguageConfig id={"hotelcategory.icon"} />}
                  required={localFields[newConstants.HOTEL_CATEGORY_GLYP_CSS].is_require ? true : false}
                />
              </Column>
              {editData &&
                ...
                localFields[newConstants.IS_ACTIVE].hasOwnProperty('value') && (
                  <Column md={3} padding={[10, 5]}center>
                    <FormControlLabel
                      ...
                      label={<LanguageConfig id={"hotelcategory.isactive"} />}
                    />
                  </Column>
                )}
              <Row>
                      ...
                      <Button
                        ...
                        <Row>
                          ...
                          <Column md={loader ? 11 : 12} xs={loader ? 11 : 12} center middle sm={loader ? 11 : 12}>
                            <LanguageConfig id={editData ? "hotelcategory.update" : "save"} />
                          </Column>
                        </Row>
                      </Button>
                      <Button
                        ...
                        variant="contained">
                        <LanguageConfig id={"hotelcategory.cancel"} />
                      </Button>
                    </Row>
                  </Column>
                </Row>
...
const HotelDetail = ({ name, value, onChange, error, helperText, label }) => {
  ...
  const hotelByKey = async () => {
    if (value && defaultOptions.length > 0 && defaultOptions.filter((f) => f.value == value).length == 0) {
      const res = await httpPostRequest(getHotelTypeY(value));
      if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
        setSetectedValue({
          label: res[newConstants.DATA][newConstants.HOTEL_TYPE_DESC],
          value: res[newConstants.DATA][newConstants.HOTEL_TYPE_KEY],
        });
      }
    } else {
  ...
  const loadDetail = async (inputValue = '', callback = null) => {
    const res = await httpPostRequest(getHotelTypeY(inputValue));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
      if (callback) {
        callback(
          res[newConstants.DATA][newConstants.HOTEL_TYPES].map((v) => ({
                          ...
                          <TableRow>
                            ...
                            <TableCell className={classes.tableBodyTuple}>
                              <TextField
                                label={"Category Name"}
                                type="text"
                                value={val[newConstants.HOTEL_CATEGORY_NAME].value}
                                name={newConstants.HOTEL_CATEGORY_NAME}
                                error={val[newConstants.HOTEL_CATEGORY_NAME].error}
                                onChange={(e) => multiStateUpdater(e, index)}
                                helperText={val[newConstants.HOTEL_CATEGORY_NAME].err_msg}
                              />
                            </TableCell>
                          </TableRow>)
                      )}
