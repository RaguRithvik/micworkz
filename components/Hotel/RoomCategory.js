...
export default function Setup() {
  ...
  const [showCol, setShowCol] = useState({
    [newConstants.ROW_NUM]: { is_hide: true, bool: true, label: <LanguageConfig id="roomcategory.sno" /> },
    [newConstants.ROOM_CATEGORY_NAME]: { is_hide: false, bool: true, label: <LanguageConfig id="roomcategory.categoryname" /> },
    [newConstants.ACTIVE_STATUS]: { is_hide: false, bool: true, label: <LanguageConfig id="roomcategory.isactive" /> },
    [newConstants.LAST_UPDATED_BY]: { is_hide: false, bool: true, label: <LanguageConfig id="roomcategory.lastupdatedby" /> },
  ...
  const loadData = async () => {
    if(!addEdit){
    setLoader(true);
    let res = await httpPostRequest(getAllRoomCategory(search_key, (currIndex - 1) * pageSize + 1, pageSize));
  ...
  const editRoomCat = async (key) => {
    setAddEdit(false);
    let res = await httpPostRequest(getRoomCategoryByKey(key));
    if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
  ...
  const delRoomCat = async () => {
    if (deleteId != '') {
      const res = await httpPostRequest(deleteRoomCategory(deleteId));
  ...
  return (
    <div>
      ...
      <PrimaryContainer
        ...
        action_key={newConstants.ROOM_CATEGORY_KEY}
        loader={loader}
        deleteLoader={deleteLoader}
        setEditData={setEditData}>
        <CustomAlert
          message={<LanguageConfig id="roomcategory.deletemsg" />}
          open={deleteId != ''}
          setOpen={setDeleteId}
          action={delRoomCat}
        />    
...
const EditContainer = ({classes, editData, setEditData, addEdit, setAddEdit, setAlertMsg   }) => {
  ...
  useEffect(() => {
    if (!editData) {
      ...
      let multi_language_ = [];
      if (editData[newConstants.ROOM_CATEGORY_LANGUAGES] && editData[newConstants.ROOM_CATEGORY_LANGUAGES].length) {
        editData[newConstants.ROOM_CATEGORY_LANGUAGES].forEach((value) => {
          multi_language_.push({
            [newConstants.ROOM_CATEGORY_NAME]: {
              value: value[newConstants.ROOM_CATEGORY_NAME],
              is_require: true,
              error: false,
              min_length: 2,
              max_length: null,
...
let multi_lang_ = multi_language.filter(f => f[newConstants.ROOM_CATEGORY_NAME].value != "")
    if (!localFields_validation.err) {
      setLoader(true);
      if (editData == null) {
        let res = await httpPostRequest(createRoomCategory(localFields, multi_lang_));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setLoader(false);
          setAddEdit(false);
          setAlertMsg({ type: 'success', msg:  res[constants.DATA_EXCEPTION].err_msg });
        ...
        let res = await httpPostRequest(
          updateRoomCategory(editData[newConstants.ROOM_CATEGORY_KEY], localFields, multi_lang_),
        );
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
          setLoader(false);
          setAddEdit(false);
      ...
      setAlertMsg({ type: 'error', msg: <LanguageConfig id="roomcategory.fillrequired" /> });
    }
  };

  return (
    <div>
      {localFields.hasOwnProperty(newConstants.ROOM_CATEGORY_NAME) ? (
        <Row padding={[10]}>
          <Column>
            <Row>
              <Column md={3} padding={[10, 5]}>
                <TextField
                  label={<LanguageConfig id="roomcategory.roomcategoryname" />}
                  name={newConstants.ROOM_CATEGORY_NAME}
                  value={localFields[newConstants.ROOM_CATEGORY_NAME].value}
                  onChange={stateUpdater}
                  error={
                    localFields[newConstants.ROOM_CATEGORY_NAME].error
                  }
                  helperText={
                    localFields[newConstants.ROOM_CATEGORY_NAME].err_msg
                  }
                  required={localFields[newConstants.ROOM_CATEGORY_NAME].is_require}
                />
              </Column>
              {editData ? (
                <Column md={3} padding={[10, 5]} center>
                  <FormControlLabel
                    ...
                    label={<LanguageConfig id="roomcategory.isactive" />}
                  />
                </Column>
              ) : (
                ''
              ...
              <Column md={6} padding={[7]}>
                <LanguageContainer languages={languages} copylanguages={copylanguages} classes={classes} multi_language={multi_language} multiStateUpdater={multiStateUpdater} />
              </Column>
            </Row>
              <Column right>
                <Row>
                      ...
                      <Button
                        ...
                        <Row>
                          ...
                          <Column md={loader ? 11 : 12} xs={loader ? 11 : 12} center middle sm={loader ? 11 : 12}>
                            <LanguageConfig id={editData ? "roomcategory.update" : "save"} />
                          </Column>
                        </Row>
                      </Button>
                      <Button
                        ...
                        variant="contained">
                        <LanguageConfig id="roomcategory.cancel" />
                      </Button>
                    </Row>
                  </Column>
                </Row>
                          ...
                          <TableRow>
                            ...
                            <TableCell className={classes.tableBodyTuple}>
                              <TextField
                                ...
                                type="text"
                                value={val[newConstants.ROOM_CATEGORY_NAME].value}
                                name={newConstants.ROOM_CATEGORY_NAME}
                                error={val[newConstants.ROOM_CATEGORY_NAME].error}
                                onChange={(e) => multiStateUpdater(e, index)}
                                helperText={val[newConstants.ROOM_CATEGORY_NAME].err_msg}
                              />
                            </TableCell>
                          </TableRow>)
                      )}
