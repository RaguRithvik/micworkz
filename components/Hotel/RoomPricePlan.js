...
export default function Setup() {
    ...
    const [showCol, setShowCol] = useState({
        ...
        [newConstants.HOTEL_SEASONALITY_NAME]: { is_hide: false, bool: true, label: "Seassionality Name" },
        [newConstants.IS_INCLUSIVE_PRICE]: { is_hide: false, bool: true, label: "Is Include Price" },
        [newConstants.EFFECTIVE_FROM]: { is_hide: false, bool: true, label: "Effective From" },
        [newConstants.EFFECTIVE_TO]: { is_hide: false, bool: true, label: "Effective To" },
        [newConstants.ACTIVE_STATUS]: { is_hide: false, bool: true, label: "Active Status" },
    ...
    const loadData = async () => {
        setLoader(true);
        let res = await httpPostRequest(getRoomPlanPrice(search_key, (currIndex - 1) * pageSize + 1, pageSize));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
            setData(res[constants.DATA]["room-seasonality-lists"]);
            setLoader(false);
            setMaxPage(
    ...
    const editRoomPlan = async (key) => {
        setAddEdit(false);
        let res = await httpPostRequest(roomPlanPriceGetByKey(key));
        if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
    ...
    async function delRoomPlan() {
        const res = await httpPostRequest(deleteRoomPlanPrice(deleteId));
    ...
    return (
        <div>
            ...
            <PrimaryContainer
                ...
                action_key={newConstants.HOTEL_SEASONALITY_KEY}
                loader={loader}
...
const EditContainer = ({ classes, loadData, editData, setEditData, addEdit, setAddEdit, languages, setAlertMsg }) => {
    ...
    const [roomplanname, setRoomplanname] = useState(editData ? editData[newConstants.ROOM_PLAN_KEY] : "")
    ...
    useEffect(() => {
        if (!editData) {
            ...
            setLocalFields({
                [newConstants.ROOM_PLAN_KEY]: {
                    value: editData[newConstants.ROOM_PLAN_KEY],
                    is_require: true,
                    error: false,
                    type: 'dropdown',
                    err_msg: '',
                ...
                [newConstants.HOTEL_SEASONALITY_NAME]: {
                    value: editData[newConstants.HOTEL_SEASONALITY_NAME],
                    is_require: true,
                    error: false,
                    min_length: 2,
                    max_length: null,
            ...
            genereteFields(editData[newConstants.ROOM_PLAN_PRICE_DETAILS])
        }
    }, [editData]);
        ...
        fields = fields.map((value) => {
            return {
                ...
                [newConstants.PAX_WISE_PRICE_DETAILS]: {
                    value: editData && value[newConstants.PAX_WISE_PRICE_DETAILS] != null ? value[newConstants.PAX_WISE_PRICE_DETAILS] : [],
                    is_require: false,
                    error: false,
                }
            }
    ...
    const save = async () => {
        ...
        if (!flag) {
            ...
            if (editData == null) {
                ...
                let res = await httpPostRequest(roomPlanPriceUpdate(editData[newConstants.HOTEL_SEASONALITY_KEY], localFields, rates, genetatedFields));
                if (res && res[constants.DATA_EXCEPTION] && res[constants.DATA_EXCEPTION][constants.ERR_CODE] == 200) {
                    setAlertMsg({ type: 'success', msg: res[constants.DATA_EXCEPTION].err_msg });
                    loadData();
                    setLoader(false);
    ...
    const generateRates = (data) => {
        let obj = {
            [newConstants.TAX_TYPE]: {
                value: data[newConstants.TAX_TYPE],
                is_require: true,
                error: false,
                type: 'dropdown',
                err_msg: '',
            ...
            [newConstants.SERVICE_TYPE]: {
                value: data[newConstants.SERVICE_TYPE],
                is_require: true,
                error: false,
                type: 'dropdown',
                err_msg: '',
            ...
            [newConstants.ADDITIONAL_TYPE]: {
                value: data[newConstants.ADDITIONAL_TYPE],
                is_require: true,
                error: false,
                type: 'dropdown',
                err_msg: '',
            ...
            [newConstants.ROOM_ALLOTMENT]: {
                value: data[newConstants.ROOM_ALLOTMENT],
                is_require: false,
                error: false,
                type: 'number',
                err_msg: '',
    ...
    const generatePax = async (val, index) => {
        ...
        if (val[newConstants.IS_PAX_WISE_PRICE][newConstants.VALUE] == false) {
            ...
            if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
                if (editData && editData[newConstants.ROOM_PLAN_PRICE_DETAILS]) {
                    let tag = false
                    for (let field in genetatedFields_) {
                        if (genetatedFields_[field].value != editData[newConstants.ROOM_PLAN_PRICE_DETAILS][field]) {
                            tag = true;
                        }
                    }
                    if (tag == true) {
                        ...
                        let newGenerateFields = editData[newConstants.ROOM_PLAN_PRICE_DETAILS].map(
                            (val, index1) => (
                                index1 == index ?
                                    {
                                        ...genetatedFields[index], [newConstants.PAX_WISE_PRICE_DETAILS]: {
                                            ...genetatedFields[index][newConstants.PAX_WISE_PRICE_DETAILS], value: val[newConstants.PAX_WISE_PRICE_DETAILS],
                                        },
                                    }
                                    : val
                            )
        ...
        <div>
            {localFields.hasOwnProperty([newConstants.ROOM_PLAN_KEY]) ? (
                <Row padding={[19, 10, 10, 10]} >
                            ...
                            <FormControlLabel
                                control={
                                    <Checkbox
                                        ...
                                        color="primary"
                                        onChange={() => stateUpdater({ target: { value: !localFields[newConstants.IS_ACTIVE].value, name: newConstants.IS_ACTIVE } })}
                            ...
                            <Row>
                                <Column md={3} padding={[10, 5]}>
                                    <TypeRates
                                        ...
                                        value={rates[newConstants.TAX_TYPE].value}
                                        isDisabled={true}
                                        onChange={(e) => objectstateUpdater(e, "text")}
                                        error={rates[newConstants.TAX_TYPE].error && rates[newConstants.TAX_TYPE].is_require}
                                        helperText={
                                            rates[newConstants.TAX_TYPE].error && rates[newConstants.TAX_TYPE].is_require
                                                ? rates[newConstants.TAX_TYPE].err_msg
                                                : ''
                                        }
                                        label={"Tax Type"}
                                    />
                                ...
                                <Column md={3} padding={[10, 5]}>
                                    <TypeRates
                                        ...
                                        value={rates[newConstants.SERVICE_TYPE].value}
                                        isDisabled={true}
                                        onChange={(e) => objectstateUpdater(e, "text")}
                                        error={rates[newConstants.SERVICE_TYPE].error && rates[newConstants.SERVICE_TYPE].is_require}
                                        helperText={
                                            rates[newConstants.SERVICE_TYPE].error && rates[newConstants.SERVICE_TYPE].is_require
                                                ? rates[newConstants.SERVICE_TYPE].err_msg
                                                : ''
                                        }
                                        label={"Service Type"}
                                    />
                                ...
                                <Column md={3} padding={[10, 5]}>
                                    <TypeRates
                                        ...
                                        value={rates[newConstants.ADDITIONAL_TYPE].value}
                                        isDisabled={true}
                                        onChange={(e) => objectstateUpdater(e, "text")}
                                        error={rates[newConstants.ADDITIONAL_TYPE].error && rates[newConstants.ADDITIONAL_TYPE].is_require}
                                        helperText={
                                            rates[newConstants.ADDITIONAL_TYPE].error && rates[newConstants.ADDITIONAL_TYPE].is_require
                                                ? rates[newConstants.ADDITIONAL_TYPE].err_msg
                                                : ''
                                        }
                                ...
                                <Column md={3} padding={[10, 5]}>
                                    <TextField
                                        ...
                                        inputProps={{ min: 0 }}
                                        disabled={true}
                                        value={rates[newConstants.ROOM_ALLOTMENT].value}
                                        name={newConstants.ROOM_ALLOTMENT}
                                        error={rates[newConstants.ROOM_ALLOTMENT].error}
                                        onChange={(e) => objectstateUpdater(e, "text")}
                                        helperText={
                                            rates[newConstants.ROOM_ALLOTMENT].err_msg
                                        }
                                        required={rates[newConstants.ROOM_ALLOTMENT].is_require}
                                    />
                                </Column>
                            </Row>
                            <Row>
                    ...
                    {localFields && localFields[newConstants.IS_PAX_WISE].value == false &&
                        <Column>
                            <Row>
                                <Column md={6} xs={6} sm={6} padding={[10]} left>
                                    <Button onClick={() => { setAddEdit(false); setEditData(null); }}
                                        variant="contained">
                                        Cancel
                                    </Button>
                                </Column>
...
const RoomPriceComments = ({ name, value, onChange, error, helperText, label }) => {
    ...
    const RoomByKey = async () => {
        if (value && defaultOptions.length > 0 && defaultOptions.filter((f) => f.value == value).length == 0) {
            const res = await httpPostRequest(RoomPriceCommentsInfoById(value));
            if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
                setSetectedValue({
