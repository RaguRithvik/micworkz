...
const HotelTagsNew=({storeTag, setStoretag})=>{
...
function handleTags (value) {
  const currentIndex = storeTag.findIndex((f) => f["hotel-tag-key"] === value["hotel-tag-key"]);
  if (currentIndex > -1) {
                        ...
                        <Column md="3" padding={[0,10]}>
                           <Box className={classes.RoomAmenitiesInformation}>
                                <li><Checkbox
                                    ...
                                    checked={storeTag.find((getval, index) => getval["hotel-tag-key"] === val["hotel-tag-key"])}
