...
const IssuesHotel=({storeIssuesHotel, setStoreIssuesHotel})=>{
...
function handleIssues (value) {
  const currentIndex = storeIssuesHotel.findIndex((f) => f["hotel-issues-key"] == value["hotel-issues-key"]);
  if (currentIndex > -1) {
    storeIssuesHotel[currentIndex]=null
    setStoreIssuesHotel(storeIssuesHotel.filter((f) => f != null));
  } else {
    setStoreIssuesHotel([...storeIssuesHotel, value])
  }
};
                        ...
                        <Column md="3" padding={[0,10]}>
                           <Box className={classes.RoomAmenitiesInformation}>
                                <li><Checkbox
                                    ...
                                    checked={storeIssuesHotel.find((val, index) => val["hotel-issues-key"] === value["hotel-issues-key"])}
                                    onChange={(e) => handleIssues(value)}/></li>
                                <Box className={classes.HotelCheckernow}>
                                    <li>{value["hotel-issues-title"]}</li>
                                </Box>
                            </Box>
                        </Column> 
                        ))}   
