...
import styles from "../NearbyPlace/Nearybyplace.module.css";
import NearPlaceType from "../NearbyPlace/NearPlaceType"
import NearPlace from "../NearbyPlace/NearPlace"
import {
    getAllNearPlace,
...
const NearbyPlaces = ({ storeNearbyPlaces, setStoreNearbyPlaces }) => {
    ...
    function handleNearPlace(value) {
        const currentIndex = storeNearbyPlaces.findIndex((f) => f["hotel-near-by-place-key"]== value["hotel-near-by-place-key"]);
        if (currentIndex > -1) {
            storeNearbyPlaces[currentIndex] = null
            setStoreNearbyPlaces(storeNearbyPlaces.filter((f) => f != null));
        }
        else {
            setStoreNearbyPlaces([...storeNearbyPlaces, value])
        }
    };
                                    ...
                                    <Paper className={styles.LanguageSlectedColorchange}>
                                        <TableContainer className={classes.TableContain}>
                                            <Table>
                                                ...
                                                {val.nearplace.map((value, index) => (value != null ?
                                                    <TableBody>
                                                        <TableRow>
                                                            <TableCell className={classes.tableHeadTuple}><div className={styles.dropdown}>
                                                                <Checkbox
                                                                    ...
                                                                    checked={storeNearbyPlaces.find((val, index) => val["hotel-near-by-place-key"] === value["hotel-near-by-place-key"])}
                                                                    onChange={(e) => handleNearPlace(value)} />{value["near-place-desc"]}
