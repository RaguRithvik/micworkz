...
import { getEventSalesReportList, getEventSalesReportView, getAllBranches } from "../../helper/RequestPayLoadReports";
...
export default function Setup() {
  ...
  const [showCol, setShowCol] = useState({
    ...
    [reportConstants.PRODUCT_NAME]: { is_hide: false, bool: true, label: "Event Name" },
  ...
  const loadData = async (loader = null) => {
    ...
    let res = await httpPostRequest(getEventSalesReportList(search_key, (currIndex - 1) * pageSize + 1, pageSize, sdate, edate, reportStatus, branch));
  ...
  const viewBtnHandler = async () => {
    let res = await httpPostRequest(getEventSalesReportView({ sdate: prepareDate(startDate), edate: prepareDate(endDate), key: branch }));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200 && res[newConstants.DATA]) {
      setViewLoader(false);
