...
import { getSalesReportList, getSalesReportView, getAllBranches } from "../../helper/RequestPayLoadReports";
...
export default function Setup() {
  ...
  const [showCol, setShowCol] = useState({
    ...
    [reportConstants.HOTEL_NAME]: { is_hide: false, bool: true, label: "Name" },
    [reportConstants.HOTEL_ADDRESS]: { is_hide: false, bool: true, label: "Address" },
    [reportConstants.HOTEL_CITY_NAME]: { is_hide: false, bool: true, label: "City" },
    [reportConstants.HOTEL_STATE_NAME]: { is_hide: false, bool: true, label: "State" },
    [reportConstants.HOTEL_COUNTRY_NAME]: { is_hide: false, bool: true, label: "Country" },
    [reportConstants.HOTEL_POSTAL_CODE]: { is_hide: false, bool: true, label: "Postal Code" },
    [reportConstants.HOTEL_EMAIL_1]: { is_hide: false, bool: true, label: "email" },
    [reportConstants.CHECK_IN_DATE]: { is_hide: false, bool: true, label: "Check In" },
    [reportConstants.CHECK_OUT_DATE]: { is_hide: false, bool: true, label: "Check Out" },
    ...
    [reportConstants.CLIENTSALEPRICE]: { is_hide: false, bool: true, label: "Sale Price" },
  ...
  const loadData = async (loader = null) => {
    ...
    let res = await httpPostRequest(getSalesReportList(search_key, (currIndex - 1) * pageSize + 1, pageSize, sdate, edate, reportStatus, branch));
  ...
  const viewBtnHandler = async () => {
    let res = await httpPostRequest(getSalesReportView({ sdate: prepareDate(startDate), edate: prepareDate(endDate), key: branch }));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200 && res[newConstants.DATA]) {
      setViewLoader(false);
