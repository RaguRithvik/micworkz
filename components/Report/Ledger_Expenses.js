...
import { getAllLedgersList, getLedgerExpenseList, getLedgerExpenseView, getAllBranches } from "../../helper/RequestPayLoadReports";
...
export default function Setup() {
  ...
  const loadData = async (loader = null, key = null) => {
    ...
    let res = await httpPostRequest(getLedgerExpenseList(search_key, (currIndex - 1) * pageSize + 1, pageSize, sdate, edate, ledger_key, branch));
  ...
  const viewBtnHandler = async () => {
    let res = await httpPostRequest(getLedgerExpenseView({ sdate: prepareDate(startDate), edate: prepareDate(endDate), key: branch }));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200 && res[newConstants.DATA]) {
      setViewLoader(false);
