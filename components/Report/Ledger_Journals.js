...
import { getAllLedgersList, getLedgerJournalList, getLedgerJournalView, getAllBranches } from "../../helper/RequestPayLoadReports";
...
export default function Setup() {
  ...
  const [showCol, setShowCol] = useState({
    ...
    [reportConstants.TXN_REC_NO]: { is_hide: false, bool: true, label: "Invoice No" },
    [reportConstants.TXN_DATE]: { is_hide: false, bool: true, label: "Date" },
    [reportConstants.TXN_TYPE]: { is_hide: false, bool: true, label: "Type" },
    [reportConstants.TXN_REC_AMOUNT]: { is_hide: false, bool: true, label: "Amount" },
    [reportConstants.MOP_REMARKS_1_CAP]: { is_hide: false, bool: true, label: "Remark 1 Caption" },
    [reportConstants.MOP_REMARKS_1_VALUE]: { is_hide: false, bool: true, label: "Remark 1 Value" },
    [reportConstants.MOP_REMARKS_2_CAP]: { is_hide: false, bool: true, label: "Remark 2 Caption" },
    [reportConstants.MOP_REMARKS_2_VALUE]: { is_hide: false, bool: true, label: "Remark 2 Value" }
  });
  ...
  const loadData = async (loader = null, key = null) => {
    ...
    let res = await httpPostRequest(getLedgerJournalList(search_key, (currIndex - 1) * pageSize + 1, pageSize, sdate, edate, ledger_key, branch));
  ...
  const getLedgers = async (callme = false) => {
    ...
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200 && res[newConstants.DATA]) {
      let data = res[newConstants.DATA][reportConstants.LEDGER_ACCOUNTS]
      setAllLedgers(data);
      if (callme) {
        setSelectedLedger(data[0])
        if (branch !== '') {
          loadData(null, data[0][reportConstants.LEDGER_KEY]);
        }
        setIsOnLoad(false);
      }
    } else {
  ...
  const viewBtnHandler = async () => {
    let res = await httpPostRequest(getLedgerJournalView({ sdate: prepareDate(startDate), edate: prepareDate(endDate), key: branch }));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200 && res[newConstants.DATA]) {
      setViewLoader(false);
