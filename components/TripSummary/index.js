...
import { Button, FormControlLabel, Checkbox, fade, Box, InputLabel, MenuItem, FormControl, Select, InputBase } from "@material-ui/core";
...
import { getReCheckRate, createBooking } from '../../helper/RequestPayLoad';
...
export default function SearchHotels() {
  ...
  const getRate = async () => {
    if (query && query.hasOwnProperty(constants.RATE_KEY) && query[constants.RATE_KEY] != null) {
      ...
      if (
        ...
        setReCheckData(res[constants.DATA]);
        setLoader(502); 
        setAlertMsg({type:'error',msg:res[constants.DATA_EXCEPTION]&&res[constants.DATA_EXCEPTION][constants.ERR_MSG]?res[constants.DATA_EXCEPTION][constants.ERR_MSG]:"Please try again later."})
      }
    }
  };
  ...
  useEffect(() => {
    ...
    if (query && query.rd) {
      ...
      adult_conf = (Array.from(Array(parseInt(adult_conf ? adult_conf : 1)).keys())).map((val, index) => ({ guestTitle: "Mr", guestTitleErr: false, guestFirstName: "", guestFirstNameErr: false, guestLastName: "", ageGroup: 'Adult', guestLastNameErr: false, guestAge: "", guestAgeErr: false, is_mandatory: index == 0 }))
      setAdults(adult_conf);
      let children_conf = query.rd.split("_")
      children_conf = children_conf.map((val, index) => index > 0 ? ({ guestTitle: "", guestTitleErr: false, guestFirstName: "", guestFirstNameErr: false, guestLastName: "", guestLastNameErr: false, guestAge: val, ageGroup: 'Children', guestAgeErr: false, is_mandatory: false }) : null).filter(f => f != null)
  ...
  function updateAdultchildrens(value, index, tag) {
    if (tag == "adult") {
      ...
      adults_update[index][value.target.name + "Err"] = adults_update[index].is_mandatory && value.target.value.length == 0 ? true : false;
    ...
    if (error_.length == 0) {
      ...
      for (let a of adults) {
        if (a.guestFirstName.trim() != "") {
          guestInfo.push({ guestTitle: a.guestTitle, ageGroup: a.ageGroup, guestAge: a.guestAge.trim() == "" ? 0 : parseInt(a.guestAge), guestFirstName: a.guestFirstName, guestLastName: a.guestLastName, guestContactNumber1: contactNoCode + " " + contactNo, guestContactNumber2: phoneNoCode + " " + phoneNo, guestEmail1: emailAddress, guestEmail2: "", guestNationality: nationalty, guestCountry: countryOfResident, isBookingPerson: contactPerson.toLowerCase().trim().indexOf(a.guestFirstName.trim().toLowerCase()) > -1 ? 1 : 0, isGuest: is_guest && contactPerson.toLowerCase().trim().indexOf(a.guestFirstName.trim().toLowerCase()) > -1 ? 1 : 1 })
        }
      }
      for (let a of childrens) {
        guestInfo.push({ guestTitle: a.guestTitle, ageGroup: a.ageGroup, guestAge: a.guestAge.trim() == "" ? 0 : parseInt(a.guestAge), guestFirstName: a.guestFirstName, guestLastName: a.guestLastName, guestContactNumber1: contactNoCode + " " + contactNo, guestContactNumber2: phoneNoCode + " " + phoneNo, guestEmail1: emailAddress, guestEmail2: "", guestNationality: nationalty, guestCountry: countryOfResident, isBookingPerson: 0, isGuest: 1 })
      }
      if(terms_conditions){
      ...
      if (
        ...
        let stripKey = 'pk_test_51HuDzuKt18HqSbGerCqjY4en5xVkUNEPtMDTZZ9zBLwv0vlGGF7m2NQj7wyR8vw4GcxamXDrQ0v9JJ1o9Aup8GOY00DD9cbuKL';
  ...
  return (!loader && loader!=502 ?
      ...
      <Card padding={[0, 0, 20, 0]}>
        <Row >
          ...
          <Column>
            <Row>
              <Column padding={[10]} color="#505ccb">
                <Text variant={"h3"} semibold component="h6" color="white">GUEST DETAILS</Text>
              </Column>
              <Column padding={[10]} >
                <GuestParentContainer adults={adults} childrens={childrens} updateAdultchildrens={updateAdultchildrens} classes={classes} />
              </Column>
            </Row>
          </Column>
          ...
          <Column>
            <Row>
              <Column padding={[10]} color="#505ccb">
                <Text variant={"h3"} semibold component="h6" color="white">CONTACT DETAILS</Text>
              </Column>

              <Column padding={[10]}>
                <GuestDetails countries={countries} dial_codes={dial_codes} hasError={hasError} stateUpdater={stateUpdater} is_guest={is_guest} phoneNo={phoneNo} phoneNoCode={phoneNoCode} nationalty={nationalty} emailAddress={emailAddress} contactNo={contactNo} contactNoCode={contactNoCode} countryOfResident={countryOfResident} contactPerson={contactPerson} classes={classes} />
              </Column>
            </Row>
          </Column>
          ...
          <Column>
            <Row>
              <Column padding={[10]} color="#505ccb">
                <Text variant={"h3"} semibold component="h6" color="white" >HOW WOULD YOU LIKE TO PAY?</Text>
              </Column> 
              <Column padding={[15, 10]}>
                <PaymentComponent paymentProvider={paymentProvider} reCheckData={reCheckData} setPaymentProvider={setPaymentProvider} classes={classes} />
              </Column>
            </Row>
          </Column>
            ...
            <Row>
              <Column padding={[15, 10]} middle> 
                <FormControl className={classes.margin}>
                  <FormControlLabel
                  control={<Checkbox checked={terms_conditions} onChange={() => setTermsAndCond(!terms_conditions)} name="Yes" color="primary" />}
                  label="I acknowledge that I have read and accept the followings" />
                </FormControl>
                <Touchable>
              ...
              <Column padding={[10]}>
                <Row>
                  ...
                  <Column md={4} center middle>
                    <Button variant="contained" style={{ width: '100%' }} color="primary" size="large" onClick={!bookingLoader ? confirmBooking : () => console.log("Your request is processing.")}>
                      <Row padding={[5, 0]}>{bookingLoader ? <Column md={1} xs={1} sm={1} center middle><Loader size={16} color="white" /></Column> : null}<Column padding={[0, 5]} md={bookingLoader ? 11 : 12} xs={bookingLoader ? 11 : 12} sm={bookingLoader ? 11 : 12} center middle><Text color="white" variant={'h3'} component={"h3"} bold>{" Confirm Booking"}</Text></Column> </Row>
                    </Button>
                  </Column>
    ...
    </div> :loader==502?<Error/> :<SkeletonContainer />
  )
}
...
const GuestParentContainer = ({ adults, childrens, updateAdultchildrens, classes, ...props }) => {
  return (
    <Row>
      <Column>
        {
          adults.map((value, index) => <GuestContainer onChange={(value) => updateAdultchildrens(value, index, "adult")} key={"adult_" + index} value={value} tag={"Adult"} index={index} classes={classes} />)
        }
        {
          childrens.map((value, index) => <GuestContainer onChange={(value) => updateAdultchildrens(value, index, "child")} key={"Children _" + index} value={value} tag={"Children"} index={index} classes={classes} />)
        }
      </Column>
    </Row>
  )
...
const GuestDetails = ({ countries, dial_codes, hasError, stateUpdater, is_guest, phoneNo, phoneNoCode, nationalty, emailAddress, contactNo, contactNoCode, countryOfResident, contactPerson, classes, ...props }) => {
  ...
  return (
    <Row>
      <Column padding={[0, 10]}>
        ...
        >
          <Column lg={3} md={3} sm={6} padding={[0,5]}>
              <TextField 
          ...
          <Column lg={3} md={3} sm={6} padding={[0,5]}>
            ...
            <InputLabel id="demo-simple-select-outlined-label">Country Of Resident</InputLabel>
              <Select 
              ...
              >
                {
                  countries.map((value, index) => (<MenuItem key={"countries_" + index} value={value.country_Code}>{value.country_Name}</MenuItem>))
                }
              </Select>
            </FormControl>
          </Column>
          <Column lg={4} md={4} xs={12} padding={[0,5]}>
            <ContactNumber
              type="number"
              name="contactNo"
            ...
            <InputLabel id="demo-simple-select-outlined-label">Nationality</InputLabel>
              <Row>
                <Column md={3} xs={4} sm={4}>
                  <Select 
                    labelId="demo-customized-select-label"
                    id="demo-customized-select"
                    error={hasError("contactNoCode")}
                    value={contactNoCode}
                    name="contactNoCode"
                    onChange={stateUpdater}
                    label="Country Code" 
                  >
                    {
                      dial_codes.map((value, index) => (<MenuItem key={"dial_code_1_" + index} value={value.dial_code}>{value.code + '(' + value.dial_code + ')'}</MenuItem>))
                    }
                  </Select>
                </Column>

                <Column md={9} xs={8} sm={8}>
                  <TextField
        ...
        >
          ...
          <Column lg={12} md={12} xs={12} padding={[5,0]}>
            <FormControl className={classes.margin}>
              <Text component="h6" variant="h6" bold>Is contact person is guest ?</Text>
              <Row>
                <FormControlLabel
  ...
  return (
    <Row {...props}>
      <Column md={6} xs={6} sm={6} padding={[10, 0]}>
        <Text size={16} bold>We accept the following payment methods:</Text>
        <Image src="/images/payments.jpg" className={classes.paymentImage} />
      </Column>
      ...
      <Column paddding={[10, 0]}>
        <Row>
          {reCheckData&&reCheckData.hasOwnProperty(constants.PAYMENT_PROVIDERS) ?
            <Column md={3}>
              <FormControl className={classes.margin}>
                <Text size={16} bold>Card Type:*</Text>
                <Select
                  labelId="demo-customized-select-label"
                  id="demo-customized-select"
                ...
                >
                  {
                    reCheckData[constants.PAYMENT_PROVIDERS].map((value, index) => (<MenuItem key={"payment_type" + index} value={value.payVendorId}>{value.payVendorName}</MenuItem>))
                  }
                </Select>
              </FormControl>
            </Column> : null}
    ...
    <Row>
            ...
            <Box className={classes.box} borderRight={1} borderColor={"#CDCECD"}>
              <Row>
                ...
                <Column padding={[10]}>
                  <Row>
                    <Column padding={[3,0]}>
                      <Text variant={"h5"} component="h6" medium size={15}><span className={classes.bold}>Check In : </span>{query && query.ci ? query.ci : "Not mension"}</Text>
                    </Column>
                    <Column padding={[3,0]}>
                      <Text variant={"h5"} component="h6" medium size={15}><span className={classes.bold}>Check Out : </span> {query && query.ci ? query.co : "Not mension"}</Text>
                    </Column>
                    <Column padding={[3,0]}>
                      <Text variant={"h5"} component="h6" medium size={15}><span className={classes.bold}>No of night : </span> {data.rooms.rates.occupancy.nights} Nights</Text>
                    </Column>
                    <Column padding={[5, 0, 0, 0]}>
                      <Text variant={"h5"} component="h6" medium size={14}><span className={classes.bold}>Room </span>{data.rooms.rates.occupancy.rooms} : {data.rooms.rates.occupancy.adults} Adult</Text>
                      <Text bold size={14}>{data.rooms.rates.occupancy.childs} : children</Text>
                    </Column>
                  </Row>
                </Column>
              </Row>
                  ...
                  <Row>
                    <Column>
                      <Row>
                        ...
                        <Column padding={[0, 5]}>
                          {data.rooms.rates.hasOwnProperty("cancelationPolicy")? <Text variant={"p"} size={14} component="h6" color="#505ccb" medium>{data.rooms.rates.cancelationPolicy.isFreeCancelation ?"Free cancellation.":"Free Cancelletion not available"}</Text>:""}
                          ...
                          {data.rooms.rates.isRefundable?<Text variant={"p"} component="h6" color="green" size={14} medium >Refund Includes</Text> : <Text variant={"p"} component="h6" color="red" size={14} medium >Refund not Includes</Text>}
                        </Column>
                      </Row>
                    </Column>
                    <Column padding={[5, 0]}>
                      <Row>
                        ...
                        <Column padding={[0, 5]}>
                          {/* <h3>{data.rooms.currency} {data.rooms.rates.netRate} </h3> */}
                        </Column>
                      </Row>
                    </Column>
                    ...
                    <Column padding={[5, 0]} center bottom>
                      ...
                      <h2 style={{ color: '#505ccb',marginLeft:5}}>{data.rooms.currency + " " + data.rooms.rates.netRate}</h2>
                    </Column>
                  </Row>
                </Column>
      ...
      <Column padding={[10, 5]}>
        <Row>
          ...
          <Column padding={[0, 10]}>
            <Text variant={"p"} component="h6" size={16}>{"Please provide the traveller's English name as per passport and don't use other special characters (%#+!@$()^*=:/?|<>). If you are reserving multiple rooms at same hotel for the same dates, please use different names for each reservation."}</Text>
          </Column>
        </Row>
      </Column>
...
const GuestContainer = ({ onChange, value, tag, index, classes, ...props }) => {
  return (
    <Row>
      <Column>
        <Row>
          <Column padding={[0, 10]}><Text component="h6" variant="h6" bold>{tag + " " + (index + 1)}</Text></Column>
        </Row>
        <Row padding={[0, 10]}>
          {value.guestTitle != "" ?
