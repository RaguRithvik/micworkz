...
import { getLanguage, b2bClientPagesGetAll, b2bClientPagesApprovedGetAll } from './RequestPayLoad';
import { httpPostRequest, httpGetRequest } from './JsHelper';
import { Snackbar } from '@material-ui/core';
import { IntlProvider } from 'react-intl';
import MuiAlert from '@material-ui/lab/Alert';
import useStoreLocal from './LocalStore';
...
export function StoreProvider({ children }) {
  const [countries, setCountries] = useState(dial_codes.map((val) => ({ country_Code: val.code, country_Name: val.name })));
  const [alert_msg, setAlertMsg] = useState({ msg: '', type: '' });
  const [headerConfig, setHeaderConfig] = useState(null);
  const [copylanguages, setCopyLanguages] = useStoreLocal('languages', []);
  const [languages, setLanguages] = useStoreLocal('languages', []);
  const [locale, updateLocale] = useStoreLocal('active_language', { label: 'English (US)', value: 'en-US' });
  const [currentAppLocale, setCurrentAppLocale] = useStoreLocal('theme_language_config', {});
  const [currentAppMenu, setCurrentAppMenu] = useStoreLocal('menus', []);
  const [filterItems, setFilterItems] = useState({});
  const [filter, setFilter] = useState({});
  const [formname,setFormname]=useState("")

  const language_image = {
    'en-US': '/images/flag/icons8-usa.png',
    'zh-CN': '/images/flag/icons8-china.png',
    es: '/images/flag/icons8-spain.png',
    fr: '/images/flag/icons8-france.png',
    it: '/images/flag/icons8-italy.png',
    'ar-AE': '/images/flag/icons8-saudi_arabia.png',
  };
  const { authUser, userBranch } = useAuth();

  const loadLanguage = async () => {
    if (authUser && userBranch && userBranch.value) {
      let res = await httpPostRequest(getLanguage());
      if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
        var lang_ = res.data.cities.map((v) => ({ value: v['language-code'], label: v['language-desc'] }));
        // setLanguages(lang_.filter((f) => !locale.label.includes(f.label)));
        setLanguages(lang_.filter((f) => f.label!==locale.label));
      }
    }
  };
  ...
  useEffect(() => {
    loadLanguage();
  }, [authUser, locale,userBranch]);


  const loadDuplicateLanguage = async () => {
    if (authUser && userBranch && userBranch.value) {
      let res = await httpPostRequest(getLanguage());
      if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
        setCopyLanguages(res.data.cities.map((v) => ({ value: v['language-code'], label: v['language-desc'] })));
      }
    }
  };

  useEffect(() => {
    loadDuplicateLanguage();
  }, [authUser, locale,userBranch]);

  const loadMultiLanguageBefore = async () => {
    let res = await httpGetRequest(b2bClientPagesGetAll(locale?.value), true);
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
      let objects = res[newConstants.DATA];
      let objects_ = {};
      for (let object in objects) {
        if (typeof objects[object] == 'object') {
          for (let inner_object in objects[object]) {
            objects_[inner_object.trim()] = objects[object][inner_object];
          }
        }
      }
      setCurrentAppLocale(objects_);
    }

  };

  const loadMultiLanguageAfter = async () => {
    let res = await httpGetRequest(b2bClientPagesApprovedGetAll(locale?.value));
    if (res && res[newConstants.DATA_EXCEPTION] && res[newConstants.DATA_EXCEPTION][newConstants.ERR_CODE] == 200) {
      let objects = res[newConstants.DATA];
      setCurrentAppMenu(objects.menus)
      let objects_ = {};
      for (let object in objects) {
        if (typeof objects[object] == 'object') {
          for (let inner_object in objects[object]) {
  ...
  useEffect(() => {
    if (authUser && authUser.is_completed && userBranch && userBranch.value) {
      loadMultiLanguageAfter()
    }
  }, [locale, authUser,languages]);




  useEffect(() => { if (authUser === null) loadMultiLanguageBefore() }, [locale, authUser,languages]);
  ...
  return (
    <Store.Provider
      value={{
        ...
        setFilterItems,
        filter, // filter user selection result
        setFilter,
        copylanguages,
      ...
      <IntlProvider locale={locale.value} messages={currentAppLocale}>
        <Snackbar
          ...
          <Alert severity={alert_msg.type} onClose={() => setAlertMsg({ msg: '', type: '' })}>
            {' '}
            {alert_msg.msg}
          </Alert>
        </Snackbar>
